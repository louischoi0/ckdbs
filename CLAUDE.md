...
| Clustered B+ tree | Built. An append-split publishes the separator before the sibling link. **A divide's walk up is not re-validated across cores** - two below-mark named inserts on two cores can land a separator in the wrong half of a parent (`docs/inflight/bugs/a-btree-divide-can-promote-into-a-parent-another-core-divided.md`, found by reading at AT-S12) | `docs/spec/heap-and-tuple.md` §5 |
| WAL | **One stream for the instance** (AR0 M0): core 0 owns the log, peers attach and append under its latch, and the superblock's `log_topology` is the word `Decode` refuses a volume on — **a pre-M0 volume no longer mounts** (AM-S4(d), on D14), and no branch below the door asks the question. Every data mutation logged (heap, undo, var-heap, index, assertions, catalog/DDL) except ALLOC/FREE and the advisory Waystone classes invariant 8 exempts. Recovery runs at mount, once per log — analysis/redo/high-water/undo, a completion checkpoint, `SHOW META`'s recovery block; a torn catalog page refuses the mount. **Every core checkpoints its own active transactions, one run at a time for the instance** (AT-S8, `wal::CheckpointGate`), and publishes into the one superblock anchor itself under the superblock latch | `docs/spec/wal.md` |
| Transactions & MVCC | Built. **Every reader is a commit-LSN snapshot over the instance read view** (AN-S2): a window of commit LSNs above a floor, one for the instance, read by every core; no view is a bound on trx ids. Live views and reader leases bound the purges instance-wide (`ReadHorizon()` is the instance's oldest live snapshot); undo pages recycle into the log's next growth. `SnapshotTooOld` is never raised; a previous run's undo pages are not reclaimed at mount. **The engine has a lock family since M2** (AO): units, modes, a wait-for graph and a deadlock detector, one table for the instance, nothing of it persisted — a writer meeting an undecided holder on its own core waits for its decide instead of being refused (one on another core is still refused retryable: the in-flight test is the core's, `known-gaps.md` Locks, found at AT-S13), and a waiter that would close a cycle is refused naming deadlock. AO-S7 measured it never fought over on its one shape, where a relation's writes ran on its owner core and the reactor serialised them; writes run wherever the session is since AT-S5 and no relation has an owner since AT-S9, so that measurement no longer describes the engine. **A read takes a borrow too since AO-S6e-b** — a positioned read declares the relation it walks and the slice it has reached, held for the statement, and it is what a `DROP TABLE` waits for; since AT-S1 it is declared **at the bind** for every relation a statement names — `FROM`, `JOIN`, subquery, a write's own — with the slice still the outermost walk's (a remote stage declared its own until AT-S9 retired the last one); the defence it gives is one-directional and `txn.md` §5 says why | `docs/spec/txn.md` §5, `docs/rules/rules.md` §3 |
| Transactional DDL | Built and durable: catalog writes logged, DDL under a real transaction, losers rolled back at mount. `CREATE TABLE` is atomic, isolated and consistent. **One name, one row on every core since AT-S17**: every name-taking write checks and writes under one hold of its catalog relation's root page, and the check refuses a name another core's open drop has freed (`catalog.md` CT7) - `DROP NAMESPACE`'s RESTRICT against a create in it is still two holds (`docs/inflight/bugs/drop-namespace-restrict-races-a-create-in-it.md`, found by reading at AT-S17); `CREATE INDEX`/`DROP INDEX` atomic and isolated on every core, both taking the relation `X` since AT-S5e - they wait for open writers and positioned readers, a writer's `IX` waits on them from any core, and a wait at the 11 s fault net is refused `TxnConflict` - and `DROP INDEX` admitted inside an explicit transaction; **`DROP TABLE` is atomic but not isolated** (§5a). **A DDL runs where the session is since AT-S5** — the ship to core 0 and `PeerDdlRefused` are gone with the route, every core writes the catalog's pages under the page latch, and a peer places a new one since AT-S5b; the delete-mark purge stays on core 0 as a **placement**, reading the instance's count. **Since AT-S2 no core is told of a DDL**: every core's catalog asks the instance's schema version word at its task boundaries, and the relation `IS` — not the word — is what keeps a stale parse from executing | `docs/spec/ddl-transactional.md`, `docs/spec/catalog.md` |
| Namespaces | A namespace is a name and **declares the affinity** of the relations created in it, which nothing reads (D18: a weight-0 hint with no consumer until AS-E). **It selected an owner core until AT-S9**, which retired placement with ownership - `PlacementPolicy`, `core_placement.hpp`, the `placement` key (refused by name). `CREATE NAMESPACE` / `DROP NAMESPACE` (RESTRICT), `SHOW NAMESPACES`, `ns.table` everywhere a relation is named. **A qualifier declares grouping, never identity** — relation names are instance-global | `docs/spec/namespace.md` (NS10) |
...
| Waystone | Recording and replay, **on every core since AT-S7**: a peer records trails and registers patterns into the instance's one `sys.patterns`, which it could not write while the catalog had one writer. `Catalog::ClaimPatternWaystoneRoot` is a claim, not a store - two cores building one pattern's directory keep the first and the loser adopts it. Trails are never retired or decayed | `docs/spec/waystone-concpets.md` |
...
| Cabin | Built, **one store for the instance since AT-S7**, partitioned by `cabin_id` (AR1 §11's second shape): every core observes into it and serves from it, and a reader holds a `CabinSet` handle rather than a pointer into the table. §4b's scope rule is struck with the per-core store that forced it - a set is authoritative for the observed value whole - and its span rule went at AT-S10 with the one walk narrower than a relation, a remote stage's slice, taking its three decline counters with it. **§6's core-ownership argument is gone and §6's announce replaces it**: a build installs an unservable empty set first, so a write on any core during the walk is in it; what the announce cannot cover the banking gate asks of the instance (§6a), and a Cabin therefore builds rarely while anything is unresolved anywhere (`known-gaps.md`). Entry sets are memory-resident; `CABIN AUTO` is the instance's one controller's, whatever core a relation's statements run on (AT-S8, AT-S9) | `docs/spec/cabin.md` |
| Secondary indexes (multi-column, covering) | Built. No index-only scan, no `UNIQUE`. **An insert re-checks its leaf's coverage since AT-S15** (a stale descent restarts, bounded, then `TxnConflict`), on the premise that no index entry is removed; **a divide's walk up is not re-validated across cores** - two cores dividing one index can land a separator in the wrong half of a parent, which since AT-S15 leaves a key range whose inserts are refused `TxnConflict` on every attempt (`docs/inflight/bugs/a-secondary-index-descent-is-not-revalidated-across-cores.md`, window 2). **A run of equal sort keys - one row whose covered column is updated ~600 times - promotes one separator twice** and refuses that row's later covered updates for good (`a-run-of-equal-index-sort-keys-promotes-one-separator-twice.md`); **a covering range probe can drop a row** whose stale entry is walked first (`a-covered-filter-after-pk-dedup-drops-a-row.md`) | `docs/spec/index.md` |
...
| Foreign keys | Declared and enforced, and **neither direction crosses since AT-S5f**: the forward check descends every parent where the statement runs, the reverse check walks every chain of every child there, and the probe protocol, both tables and the reference intent retire with them. The forward hoist stays, for deduplication and because the wait has to happen before any row is written. **A parent being written is waited for on whichever core holds it** — the wait is taken on the parent row's own entry in the instance's lock table and acquires nothing, so it is not D9(a)'s fence; the waiter is an edge in the wait-for graph. The reverse check answers busy on an uncommitted child, which is the evidence the intent stood in for — **for the interval after the child's write.** The interval before it is **open across cores**: a forward check that passes holds nothing, so a parent can be deleted between the check and the child's write, which the reference intent used to prevent and which D9(a)'s `S` fence is what closes (`known-gaps.md`, Foreign keys). **A Cabin set may find a child and may not clear one**: the rule was made for the per-core store, the store is the instance's since AT-S7, and the clearing return is not restored - restoring it is undecided. CASCADE/SET NULL refused | `docs/spec/foreign-keys.md` |
| Assertions | Enforcing on every core, including immediately after a restart. **One registry for the instance since AT-S5d**: `CREATE ASSERTION` builds where its session is, every core's writes check and reserve into the one directory each assertion has, under the registry's latch, and an admission holds its contribution until its reservation is logged; core 0's mount resumes it. An assertion the registry cannot enforce refuses the relation's writes on every core. **A build holds the relation `X` since AT-S5e**, under a transaction of its own, so it waits for open writers and every writer arriving during it waits for the adoption; a wait at the 11 s fault net is refused `TxnConflict` | `docs/spec/assertion.md` §6.1 |
| Access statistics | `SHOW ACCESS`. **Written where the statement ran since AT-S7**: CR7's fold into an `AccessBatch` and its flush to core 0 are retired with ring kind 39 and CR8's permitted drop, and `Catalog::RecordAccess` holds `sys.access_stats`' root page exclusive so the fold and the admission of a new shape are one act across cores | `docs/spec/heap-and-tuple.md` §7, `docs/spec/crosscore.md` CC13 |
| ALTER TABLE | Catalog-only: RENAME TO / RENAME COLUMN; assertions RESTRICT; everything data-moving refused | `docs/spec/alter.md` |
| DROP TABLE | Catalog-scoped: oid tombstoned and never reissued, pages orphan; fkeys/assertions RESTRICT. **Since AO-S6e-b the drop takes the relation in `X`** — it waits for a positioned reader and for an open writer of the relation, the wait is bounded by the lock family's 11 s fault net, and under a continuous stream of readers the drop can be refused `TxnConflict` where it used to succeed. Atomic still, isolated still not | `docs/spec/drop-table.md` |
...
| Physical optimizer, Part II: Cabin controller | Built, with `SHOW CABIN_OPTIMIZER`; managed state is memory-resident. **One surface for the instance since AT-S8** (`OptimizerSurface`): every core's `SET CABIN_OPTIMIZER` flips the one switch, every core feeds the one latched collector and reads the controller under its view latch | same, Part II |
| Buffer-pool eviction | Every accessor returns a pinned `PageRef`; the CLOCK sweep runs on the fault path under `buffer_pool_frames` (default 0 = unbounded). **Writeback copies each page under its page latch and cleans only what the copy covered since AT-S8 step 1b** (a per-frame dirty generation) — before it, one core's checkpoint could persist half of another core's write and drop the mark of one that had not gone out. **One pool for the instance since AM-S2 step 3** — a peer borrows core 0's frame table rather than opening its own, so `buffer_pool_frames` is an undivided total and a page faulted on one core is served from the frame another loaded. **Unconditional since AM-S4(d)**: the sharing was gated on the volume having one WAL stream, because the writeback gate is a property of the log, and a volume with more than one is now refused at mount | `docs/spec/eviction.md`, `docs/spec/page.md` §3, `instructions/v3.0.0/workorder-am-m1-shared-pool.md` |
| Cross-core execution | **Every statement runs on the core its session is on**: writes since AT-S5, reads since AT-S6, and since AT-S9 with nothing left that chose another core - the single-step fan-in over a split relation and the two-step join pipeline retired with ownership (the second was also wrong: inside `BEGIN` its stages could not see the transaction's own writes). Statement shipping and the cross-owner transaction retired at AT-S6 (ring kinds 28, 29, 33-38 struck), and the lock family's fault net is 1 s (`lock_wait_fault_net_ms`). **Since AT-S10 nothing crosses**: the remote-step protocol - its server, the session's step client, the step descriptor codec and ring kinds 1-6 and 40 - is deleted (AT-0 item 4, answered *struck*). **Since AT-S10b the ring carries nothing the engine sends** (every core carves its own transaction-id window and bumps a relation's row-id mark in place), **and since AT-S10d there is no ring**: the transport, its header and kind enum, phase 3 and `AttachTransport` are deleted, and the one cross-core path is write-then-kick through the wake registry - best-effort, a skipped kick costing one idle block. `SHOW META` lost `sched_wake_race_skips` and `sched_spurious_wakes` with it | `docs/spec/crosscore.md`, `docs/spec/sched.md`, `docs/spec/cross-owner-txn.md` (retired) |
| Task representation | C++20 stackless coroutines | `docs/spec/sched.md` §3 |
| Wire protocol KWP/1 | **Every core accepts on the default port since AT-S8** (SO_REUSEPORT above one core; a session runs where it was accepted, under one TLS/SCRAM/limits setup; `peer_listeners` retired and refused). **Where the platform refuses SO_REUSEPORT (D19's fallback, AT-S10c)** core 0 binds alone, accepts every connection and hands each round-robin to a core by an inbox and a kick, and the session runs where it was handed (`include/kds/server/connection_handoff.hpp`). The default port speaks length-prefixed binary frames: handshake with capability intersection, SCRAM, PARSE/BIND/EXECUTE over statement and portal handles, typed row batches, portal suspension, transaction and durability frames, a pinned error registry. `kMaxFrame` 16 MiB, 64 KiB batch target, 60 s portal-idle timeout, 64 statements and 64 portals per session; `max_rows` bounds delivery, not execution; `C_DESCRIBE` of an unexecuted statement is refused. The newline protocol is `debug_text_port`'s loopback debug surface, off unless configured; `STOP` is reachable only there. TLS 1.3 and the statement-class roles are off by default; the `COMPRESSION` bit is reserved and never offered | `docs/spec/protocol.md`, `docs/spec/client-manual.md` |
...
| Ranges and core ownership | **Retired at AT-S9** (AR0-5 D17, the operator's ruling on spreading). `sys.tables.owner_core` and `sys.ranges.owner_core` are reserved bytes - written 0, never read, so a pre-AT volume mounts unchanged - and **nothing names a core on disk**, so the core count is not pinned: a mount at another `cores` records it. No range is opened (`range_size_ids` refused by name); a relation split before AT-S9 keeps its ranges, each its own chain, and is read and written whole on whichever core the session is on. Every multi-owner refusal retired with the owners it described | `docs/spec/crosscore.md` (CC7-CC10), `docs/spec/heap-and-tuple.md` §4.1a |
| Stride forest | Rejected | `docs/spec/crosscore.md` §6b |
| Observability | Not a subsystem. Two `SHOW META` blocks: per-scheduling-group accounting against reactor wall time, the idle policy's counters. The cross-core write refusal counters retired at AT-S9 with ownership | `docs/spec/sched.md` §4, `docs/spec/crosscore.md` §6 |
...
11. **Every** relation's pk is a unique 40-bit id, carried only by the Keystone word, never rebound, **never updatable**. First column must be integer-typed. Where the id comes from is a per-**row** fact — the `INSERT` names it or omits it — and there is no key mode. `sys.tables.next_id` is a high-water mark on what has been placed; uniqueness follows from it with no page read for an omitted key and for a named key at or above it. A named key **below** the mark is btree-only, proved by the descent, and refused `OutOfRange` on a heap relation. Monotonicity is per-relation and per-history, never engine-wide, and `sys.tables.key_order` is the only truthful reading of it (`docs/spec/heap-and-tuple.md` §4.1). Insert spreading is retired (AT-S9). The pk is an identity **and a sequence**, monotonic in issue order, however many cores insert — every core bumps the one mark under its page latch (§4.1a). Uniqueness and `key_order` hold, and invariant 3 holds per range on a relation split before AT-S9.
