...
| Namespaces (the placement declaration) | **Built 2026-09-02** (AF-T0..AF-T4, operator ratification AF). A namespace **selects the core** that owns the relations created in it, fixed by its first relation and never rebalanced — `PlacementPolicy::kNamespace`, the **shipped default**, which answers exactly what `kCreatingCore` answers for a relation in `public`, so DA2 is unreversed for anyone who declares nothing. Surface: `CREATE NAMESPACE` / `DROP NAMESPACE` (RESTRICT), `SHOW NAMESPACES`, and qualified names `ns.table` everywhere a relation is named. **A qualifier declares placement, never identity** — relation names stay instance-global, so `ns.t` *selects* at `CREATE TABLE` and is *verified* everywhere else; `docs/spec/namespace.md` NS5/NS6/NS8's namespace-scoped names are declined and marked in place. No format bump: `namespace_oid` has been on `SysObjectRow`/`SysTableRow` since the format existed, and `kFingerprintVersion` does not move (a keyword hashes as an identifier; `ns.t` is not a new token shape). **Measured** (`bench/v2.8.0/results-af-t5-namespace-grouping-*`, eight cells): against single-core placement a declared grouping is **1.12x at two groups, 1.38-1.44x at three and 1.95x at seven**, and **0.81x at one** - the hop is real and only parallelism pays for it. Against *blind* rotation the answer is **conditional on declared groups per writer core**, which is the sweep's finding: **1.22x at 2.33** groups per core and **0.90x at 1.00** - keeping a wired pair together pays once cores are shared and pays nothing while every group can have one. AF-T5's own instruction - re-run DA2's cell - could not be followed and that is a finding too: that cell's workload has no join and no foreign key, so nothing in it ever crosses. **And co-location has a measured write-side cost whose mechanism is unknown**: a seven-group load ran **2.35x** slower co-located than scattered, collapsing to 1.02x under `relaxed`. The first explanation - `group` durability's batch falling to one - was **published and retracted the same day**: `SHOW META`'s new `wal_mean_group_batch` reads **1.000 at one *and at two* committers on a core**, so the batch is not what separates the layouts - and the sweep behind that is the bigger finding: **D2 does not batch below four concurrent committers on a core** (1.000, 1.000, 2.000, ~4.000 at 1/2/4/8), so a core with one or two pays D1's device cost. Open: namespace-scoped names; an **empty** namespace's core across a mount that changes `cores`; anything past 2.33 groups per writer core; and how that write cost scales | `docs/spec/namespace.md` (NS10 is the rule), `instructions/v2.8.0/ratification-af-namespace.md` |
...
| Cabin (value-observed authoritative store) | v1 built (CB01-CB11); CB12-CB14 add the correlated probe, EXISTS convergence and per-key observation — the one *banked* acceleration a heap relation's join column can have; entry sets memory-resident. **§6a**: a set is banked only from a read view nothing can contradict — no in-flight transaction, and not inside one. **A store per core since 2026-09-02** (AK-S2, `instructions/v2.8.0/workorder-ak.md`): core 0 held the engine's only store, so a namespace-placed relation's Cabin served nothing and the funding gate refused every write to it; now the relation's owner observes, appends and serves, and the gate's Cabin arm is gone. The optimizer's signals stay core 0's, so a peer-owned relation earns no `CABIN AUTO` — named, not fixed | `docs/spec/cabin.md` |
...
| Foreign keys | Declared and enforced (FK-M1..FK-M5); CASCADE/SET NULL out of v1. **Cross-owner since 2026-09-02** (AH, AI): a child and its parent may live on different cores - the forward check probes the owner from the dispatch fork, and **the reverse fans out since 2026-09-02** (AJ-T3 for `DELETE … WHERE pk = k`; AK-S3 for every other `WHERE`, which collects the pks by a read-only pass first and takes as many probe rounds as the set needs), so a cross-owner parent can be `DELETE`d. The crossing costs +5.8% of a write under `group` and is flat in the number of owners, because the fork parks once over all of them | `docs/spec/foreign-keys.md` |
...
- **Cross-core** (`docs/spec/crosscore.md` §9, `docs/spec/sched.md` §10):
  ...
  RD3's resolver); snapshot forwarding to the
