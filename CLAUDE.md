...
- **Protocol** (`docs/spec/protocol.md` §14): ~~frame/batch/timeout
  ...
  says so). Still open: the **idle-session** timeout, which is a different
  quantity — nothing bounds an authenticated connection sitting with no
  statement and no portal; ~~compression~~ — **deferred by KW-D5**, the bit
  reserved and unoffered, and adding it later is not a version break;
  credit/window flow control, which §7's explicit `max_rows` suspension
  replaces until something needs more; `STOP` as a capability-gated admin
  statement (**deferred by KW-D4**, with the consequence that `STOP` now
  lives only on the debug port); and per-relation grants, gated on catalog
  recovery. (TLS §1, SCRAM §14 and statement-class authorization §14 all
  landed 2026-08-13; SCRAM's carriage into KWP's handshake frames landed
  2026-08-31 with P07 and took the `[PROPOSED shape]` with it.)
