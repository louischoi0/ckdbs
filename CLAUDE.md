...
| Transactional DDL | **Spec only, nothing built** (2026-08-15). Reverses `docs/txn.md` §9's "out of scope" by direction, and §7/§9 are amended to say so. Scope is **atomicity, isolation and consistency** — catalog rows take the real trx id, catalog reads filter through the visibility predicate, so rollback needs no undo record. **Durability is deferred by name** (catalog writes stay unlogged and unrecovered, RV3), so nothing may claim crash-durable DDL yet. DT1 done; DT2 next. Open: the cache strategy, and concurrent DDL of the same name | `docs/spec-ddl-transactional.md`, `docs/workplan-ddl-transactional.md` |
