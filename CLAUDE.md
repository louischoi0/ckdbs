...
| Transactions & MVCC | Built. **Every reader is a commit-LSN snapshot over the instance read view** (AN-S2): a window of commit LSNs above a floor, one for the instance, read by every core; no view is a bound on trx ids. Live views and reader leases bound the purges instance-wide (`ReadHorizon()` is the instance's oldest live snapshot); undo pages recycle into the log's next growth. `SnapshotTooOld` is never raised; a previous run's undo pages are not reclaimed at mount. **The engine has a lock family since M2** (AO): units, modes, a wait-for graph and a deadlock detector, one table for the instance, nothing of it persisted — a writer meeting an undecided holder waits for its decide instead of being refused, and a waiter that would close a cycle is refused naming deadlock. It is taken and released and, on the one shape AO-S7 measured, never fought over: a relation's writes run on its owner core and the reactor serialises them. **A read takes a borrow too since AO-S6e-b** — a positioned read declares the relation it walks and the slice it has reached, held for the statement, and it is what a `DROP TABLE` waits for; since AT-S1 it is declared **at the bind** for every relation a statement names — `FROM`, `JOIN`, subquery, a write's own — and by a remote stage for itself, with the slice still the outermost walk's; the defence it gives is one-directional and `txn.md` §5 says why | `docs/spec/txn.md` §5, `docs/rules/rules.md` §3 |
