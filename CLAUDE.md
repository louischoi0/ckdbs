...
| Foreign keys | Declared and enforced, cross-owner in both directions: the forward check probes the parent's owner from the dispatch fork, the reverse fans out over as many probe rounds as the pk set needs. **A parent being written is waited for on whichever core holds it** (AO-S3 same-core, AO-S5(b) across cores: the parent's core parks the probe until the writer decides, up to the probe's deadline, and the wait is an edge in the instance's wait-for graph; a row the transaction's own participant wrote is its own and parks nothing, and a decide that reaches a parked probe abandons it); the reverse probe still answers busy. CASCADE/SET NULL refused | `docs/spec/foreign-keys.md` |
