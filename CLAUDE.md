...
| Foreign keys | Declared and enforced (FK-M1..FK-M5); CASCADE/SET NULL out of v1. **Cross-owner since 2026-09-02** (AH, AI): a child and its parent may live on different cores - the forward check probes the owner from the dispatch fork and the reverse refuses by name, so a cross-owner parent cannot be `DELETE`d until the fan-out is built. The crossing costs +5.8% of a write under `group` and is flat in the number of owners, because the fork parks once over all of them | `docs/spec/foreign-keys.md` |
