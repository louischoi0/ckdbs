...
| WAL | **One stream for the instance** (AR0 M0): core 0 owns the log, peers attach and append under its latch, and the superblock's `log_topology` is the word `Decode` refuses a volume on — **a pre-M0 volume no longer mounts** (AM-S4(d), on D14), and no branch below the door asks the question. Every data mutation logged (heap, undo, var-heap, index, assertions, catalog/DDL) except ALLOC/FREE and the advisory Waystone classes invariant 8 exempts. Recovery runs at mount, once per log — analysis/redo/high-water/undo, a completion checkpoint, `SHOW META`'s recovery block; a torn catalog page refuses the mount | `docs/spec/wal.md` |
