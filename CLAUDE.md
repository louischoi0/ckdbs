...
| Cross-core execution | A two-step join executes across cores and a split relation fans in, replies byte-identical to local execution. **No statement crosses since AT-S6**: a read runs where the session is, as a write has since AT-S5, so statement shipping and the cross-owner transaction are retired whole - `MayShip`, the enrolment, the participants, both phases and the in-doubt resolution, with ring kinds 28, 29 and 33-38 struck. A transaction is one core's, whole, so `REPEATABLE READ` reads one instant by having one core to read it on, and the fault net that had to clear a coordinator's 10 s phase deadline falls to 1 s (`lock_wait_fault_net_ms`, `in_doubt_ceiling_ms` re-scoped and the old spelling refused at startup). What still crosses is a **stage**: the fan-in over a split relation and the two-step join, whose answer edge carries typed rows (§4a) | `docs/spec/crosscore.md`, `docs/spec/sched.md`, `docs/spec/cross-owner-txn.md` (retired) |
