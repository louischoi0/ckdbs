...
| Range-granular core ownership | The end-state for dynamic core allocation: pk-range ownership units, Waystone/Cabin as the routing layer, id-block-aligned insert spreading, CC7's handoff as the mover. **R0 closed 2026-08-24** — PL ratified (PL-B handoff + PL-C stamp, `docs/spec-page-lsn-cross-stream.md` §9). **The rules promoted 2026-08-24** (worktree `v2-crosscore-range-rules`): `docs/crosscore.md` v2 owns the unit, directory, routing, write scope, split gates and migration contract (CC8-CC10, §2a, §5-§6b); the blueprint keeps the phasing and thesis. Phases R1-R6 remain (R1 underway via `docs/workplan-peer-writer.md` PW4/PW5; R2's static half built). **The PL contract's first consumers built 2026-08-24** (worktree `pw1c1-handoff-record`): PW1c-1 — `kPageHandoff` and its one emitter; PW1c-2 — analysis removes a handed-off page from the dirty page table and redo's new not-dirty filter never faults it, closing the finding that redo never consulted that table at all; PW1c-3 — the PL-C stamp rides every `StampPageLsn` and redo apply, and **a reachable foreign stamp refuses the mount unconditionally**: a same-day rule 5a (a scan-window bypass for the returned page) was retracted by its own review — it falsely refused the healthy receiving core — and §9 rule 6, the durable acquisition restamp, replaced it; **PW1c-4 built the same day**: exact-page write grants end CC7's write-unsafe superset, the publish sends handoffs-durable-then-grants, and the receiver logs its own acquisition `PAGE_HANDOFF` and restamps with that record's LSN before any write is admitted — and **PW1c-5 the same day: the first funded peer INSERT runs end to end** (interim guard replaced by the shape gate + always-on leased-store `MayWrite`; the PW1c-4 review's C1 fixed — grant receivers refresh the peer's free-map snapshot, without which every post-startup relation's grant was dropped). **PW1c-6 closed as the refusal half the same day** (`CREATE INDEX` on a peer-owned relation refused with task and byte; the grant extension deferred to PW2, where it buys something); re-grant-at-mount and PW2 remain, then PW6's benchmark | `docs/blueprint-range-ownership.md`, `docs/crosscore.md` |
...
- **Cross-core** (`docs/crosscore.md` §9, `docs/sched.md` §10): the 2PC
  ...
  interleave default; the shared-structure access mechanism.
