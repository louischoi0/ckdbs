...
| AJ-T3 | **Built 2026-09-02** (`6de7c51`): the fork hoist for `DELETE … WHERE pk = k`, the register-then-check-then-fan-out order, the park, the resume, the per-row check from held verdicts. Landed with the two refusal cells rewritten and **no driven end-to-end cell**; that cell is `ACrossOwnerParentDeleteByPkFansOutAndDeletes`, added by AK-S3 the same day |
| AJ-T6 | **Built 2026-09-02 as AK-S3** (`instructions/v2.8.0/workorder-ak.md`), on the operator's "remove all refusals" rather than a benchmark's ask: the collect pass under the statement's snapshot, and — what this order's text did not foresee — `DispatchAsync`'s one-park-per-dispatch refusal turned into a bounded round loop, because a collected set outgrows one message per owner and a row can appear while the statement is parked. §3a rewritten |
| AJ-T4, T5 | not started |
