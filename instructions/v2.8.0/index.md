...
| `workorder-ak.md` | **The auxiliary refusal matrix, and every refusal taken down that consistency lets go.** The operator's 2026-09-02 direction — remove all auxiliary refusals keeping consistency, rule, functionality and concept — filed *under* AE: the range-shaped rows stay by AE-5.1 (AK-R0 asks the operator to confirm), the owner-shaped and feature-shaped rows are staged AK-S1..S10, cheapest divergence first. Every row carries its site at `464b6a0`, why it exists, and the mechanism that replaces it; two stages (heap parents, nullable index columns) reverse a ratified fence and proceed under AK-R2/AK-R3 unless the operator says otherwise |
