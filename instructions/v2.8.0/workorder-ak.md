...
| AK-S3 | **Built 2026-09-02** on `worktree-resolving-refusal-window` — AJ-T6, and one thing AJ's text did not foresee. `HoistReverseForeignKeyChecks` takes a `collect` callback: a non-pk `WHERE` collects its pks by a read-only pass under the statement's snapshot (`DeleteInner`'s stage-1 match, factored into `qualifies` and shared with the mark), registers every one in the pending-delete set before the fan-out, asks the intent table per pk, and groups at most `kFkReverseProbeMaxEntries` questions per owner per round. **The one-park-per-dispatch refusal in `DispatchAsync` is gone**: the probe park is a loop bounded by `kFkProbeMaxRounds`, the resumed verdicts accumulate across rounds, and a resume that parks again — a set past one message, or a row that became visible while parked — is another round rather than `NotImplemented`; past the bound the answer is `TxnConflict`. A collected pk the walk meets with no answer is refused retryably (`FkParentVerdicts::collected()`), never marked. Cells: the pk shape driven end to end (AJ-T3's owed cell), the predicate shape (three deleted, the referenced one `FK_VIOLATION`, an empty set `DELETED 0`), a set of `kFkReverseProbeMaxEntries + 5` rows taking a second round, and a row inserted while the statement is parked caught by the next round (`DELETED 2`). `foreign-keys.md` §3a rewritten as the fan-out it now is. Cost named: one extra walk of the parent on the non-pk shape, only when a child is on another core. Suite 3205/3205 at `94a0b55`. **Review** (`critics-developer`, after the push): the mechanism's shape held — the register-before-fan-out over every collected pk, the per-round intent check, the per-group cap, termination, and the `qualifies` refactor all verified — and it found **one blocking defect that predates S3 and that S3 rests on**: `HandleDelete` never got the `pending_fk_probe` arm its INSERT and UPDATE siblings have (AJ-T3 at `6de7c51` did not add it), so a parked DELETE fell through to `EndWrite(statement_ends = true)`, committed an empty transaction per round and released its pending-delete registration at the park — a child could then be committed against a parent about to go, RESTRICT violated silently. Fixed in the follow-up commit with the arm, and pinned by `live_rows() == 1` while parked. Three more applied: the resumed-verdict members were live across a `co_await` for the first time (another statement on the core would read them as its own) — the answers now accumulate in the coroutine's frame and land on the members only around the synchronous resume, and the reply collection moved to `CollectProbeReplies`; the round cap `kFkProbeMaxRounds` turned a deterministic size limit into a retryable refusal that could never succeed — gone: the rows are now **fixed by the first round** (`PendingFkProbe::rows`, `resumed_fk_rows_`) and carried through every re-entry, so the loop is finite by construction and a row that appears after the pass is refused retryably rather than re-collected; and `FkParentVerdicts` was a linear scan now holding thousands of entries — a `std::map`. Two cuts declined: folding the hoist's two `pks × foreign` passes into one (it reorders register-vs-group, which AJ-R3(a) forbids without a fresh argument) and a per-child `InitTableAccess` hoist (a cache hit per round) |
| AK-S4..S10 | not started |
