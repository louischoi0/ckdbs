...
| XG1 | **Request side built 2026-09-01, gated; the owner's half is not.** The `form` byte with its fail-closed decode, the 16-byte `PipelineTag` on the request POD (`kShippedStatementTextMax` **976**, asserted where derived), `SessionStepClient::RegisterInbound`, `ShipStatement`'s mint/register/ship and its close-on-refusal, and the tag's carriage across the park. **`kShippedTypedAnswerBuilt` is `false`** - the refusal stays until the owner can install a batch sink, send the description and stream the rows, because shipping `form = 1` to an owner that renders text would answer a typed client with a rendered line, which is the failure the refusal exists to prevent. Remaining: the owner's sink, the description message and its chunking, the credit-governed send, the arrival core's forward, and XG-R2's dedup arm. Status table and one finding in `workplan-shipped-read-typed.md` §8a |
