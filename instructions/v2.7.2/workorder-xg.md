...
| XG1 | **Both wires built 2026-09-01, still gated on one decision.** Request side: the `form` byte with its fail-closed decode, the 16-byte tag (`kShippedStatementTextMax` **976**), `SessionStepClient::RegisterInbound`, `ShipStatement`'s mint/register/ship. Owner side: `WireResultSink` reused with a settable byte target sealed at `StepBatchCeiling`, the answer edge as a fifth producer on the existing pipeline (`OpenAnswerEdge`/`PushAnswerBatch`/`CloseAnswerEdge` - credit, `Drain`, cancel and EOF unchanged), `kShippedRowDesc` with its own header and sequence chunked to its own ceiling, and XG-R2's dedup exemption on the typed arm. **`kShippedTypedAnswerBuilt` is still `false`**: what remains is the arrival core's forward into the session sink, and it carries a decision the `ResultSink` seam does not answer - XG-R1 says forward without re-encoding, which is right for `WireResultSink` and wrong for `TextResultSink` through the same `Emit`. Three options priced in `workplan-shipped-read-typed.md` §8b, CLA recommends the second |
