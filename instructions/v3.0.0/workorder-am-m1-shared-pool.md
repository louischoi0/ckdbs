...
| AM-S2 | **Not started. Surveyed 2026-09-05** on `worktree-ar2-borrow-model-2` at `ecf17cc`, and the survey moves the stage's shape rather than confirming it. **What exists today: one store per core, none of it synchronised.** `CoreRuntime::Open` opens its own `DevicePageStore` over the shared device (`src/server/core_runtime.cpp:163`) beside core 0's (`src/server/expeditor.cpp:741`), so at `cores = N` there are N frame tables over one file. Inside a store, **only the page-latch word is atomic** (AM-S1): `frames_` is a plain `std::unordered_map<PageId, Frame>` (`device_page_store.hpp:1153`), and `clock_hand_`, `frame_budget_`, `live_pins_`, `pin_high_water_` and `dirty_eviction_queue_` are plain members beside it. `Frame::pins` is plain too, and its comment already names this stage as the owner of that change — "the latch word above is the one field a shared pool will read from another core; the pin count is not, and **AM-S2 owns that change**" (`:822-829`). So AM-S2 is not "share the table"; it is "give the table a concurrency protocol it has never had". **The one constraint the survey removes.** A pinned page's bytes are stable under *any* frame-table reorganisation: `PageRef` holds `store_`, `page_id_` and a raw `data_` pointer rather than a `Frame*` (`page_store.hpp:93-100`), and `Frame::bytes` is a `unique_ptr<Page>` whose allocation the table never moves. So the table's representation is free — a rehash, or EV05's open addressing (`page.md` §16-7) — and the protocol only has to cover `Frame` *metadata*, never page data. The first draft of this row assumed the opposite and would have ruled out the open-addressed form the eviction plan already schedules. **The budget is divided today and the two sites agree**: `FrameBudgetShare` (`expeditor.cpp:124`) is applied to core 0's store at `:749` and written into each peer's config at `:1692`, and `CheckFrameBudget` (`:113`) refuses `frames < cores` because a share of zero would read as unbounded. Both go when the total stops being divided — the row's "the boot refusal for `frames < cores` goes" is that refusal, and it has one cell and one manual line behind it. **What AM-S2 owes as rulings, none of them answered here**: (R-a) **what guards the table.** Proposal: one structure latch over insert/erase/lookup, held across the map operation and **never across the device read** a fault performs, plus `Frame::pins` becoming atomic; sharding is EV05's, not this stage's. (R-b) **the page-against-page order**, which AM-S1 explicitly left unstated and this stage inherits — the census AM-S1 ran already names every site that holds two page latches (chain growth, the btree and index leaf re-fetch), so the proposal is a total order by page id over those, not a general rule nobody can check. (R-c) **the CLOCK hand under sharing**: one hand, advanced under the structure latch, with the O(n log n) re-find per pass that `:1144-1147` already calls out as EV05's to fix. (R-d) **what a `CoreRuntime` holds** — a reference to the instance's store rather than a `std::unique_ptr` of its own (`core_runtime.hpp:548`), which is the change that makes every other core's page reachable and is therefore the one that turns AM-R2's `MayFault` removal from a deletion into a behaviour change. **Not started deliberately.** This is a concurrency protocol for the engine's hottest structure, and this session has already shipped one silent wrong answer (`UPDATED 0`, `07c94c7`) by building fast and pushing before a review returned. The stage wants a fresh session with the rulings above marked first, not a tail-end pass |
