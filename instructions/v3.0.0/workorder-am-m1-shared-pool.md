...
| AM-S1 A/B | pending — the operator chose to run it despite `CLAUDE.md`'s suspension: `tools/scenario0_stockmarket.py` at `cores = 1`, `group` and `strict`, interleaved between hashed release copies at the parent commit and the landing commit, per `bench/README.md` |
