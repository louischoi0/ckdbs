...
(b) and AM-S1 are done; AM-S2..S6 have not started.** AM-1's "must not start before AL-S8's numbers are read" was
...
gains a row. (It said "its fourth" until AO-S8; `rules.md` §3 assigns no ordinal, and four documents had claimed this one.)
...
| AM-S4, what happened | **Landed as AW-S1b in two halves, 2026-09-06 and 2026-09-07** (`workorder-aw-m1-close.md` §9). The refusal half first — `BootstrapDatabase`'s `log_topology` parameter still accepted `kPerCoreStreams`, so the arrangement `16e6c5c` refused was reachable from tests; the parameter and its three cells went. Then the deletion, and **the four slices this row implies are one surgery**: `MayFault` is `TryClaimByStamp`'s read trigger and the claim is load-bearing while the lease exists, so cutting either alone leaves the suite red and only cutting the lease with them makes it green. Everything named above is gone, plus the write grants, the whole CC7 publish hook, `RefreshFreeMapFromDevice`, `AdoptDeviceMapOnMiss`, the extent-lease and relation-grant services, and `storage::Extent` itself. **The sizing was M and it is L**: 44 cells left with the machinery, and `CoreRuntimeTest`'s fixture had to start sharing the pool because it was the last place in the tree still giving each peer a private store. **Two things this row asks for did not land**: the stamp *field* stays (redo reads it; only its ownership reading went) and `page-lsn-cross-stream.md` is still in the tree | as stated, less those two | L, done but for the per-core-stream mount branches |
| AM-S4(d), **and AM-S4 is closed** | **Landed 2026-09-07** on `worktree-am-s4d-s3-close`, in two commits: `5e8b041` the door, `40510cc` the collapse. **The door was the whole difficulty and it was not where this row put it.** AM-R4a's chain is *D14 makes every mountable volume one this build created, and every such volume is single-stream* — AW-S1b closed the bootstrap hole in that chain, but `Decode` still **accepted** `kPerCoreStreams` inside a version-17 image, so the branches were reachable in principle and could not be deleted as dead code. `Decode` refuses it now, naming the absent migration. **The larger half was `CreateFresh`**, whose `log_topology` parameter defaulted to `kPerCoreStreams`: every superblock built outside `bootstrap.cpp` — fifty-odd, all in tests — claimed the topology the door refuses, which is how five anchor cells were quietly asserting a per-core table. The parameter is gone. Then the collapse: `CoreRuntime`'s two mount arms with `log_device_` and `Config::wal_dir`, `Expeditor`'s five sites, `SetWalAnchor`'s slot rule, the anchor fold, redo's foreign-stamp refusal and restamp, analysis's handoff erase, `AnalysisStart::single_stream` and `Redo`'s parameter, and **`CoordinatorStreamResolver` with `wal::PreparedResolver` and the whole cross-stream prepared-resolution phase** — checked first that the live in-stream arm carries its own three cells, so what went is coverage of the dead arm. `single_stream()` and `log_topology()` are deleted on the dispatcher's own argument about `wal_anchor_count`: a predicate that can only answer true is the defect a field that can only say one thing is. `page-lsn-cross-stream.md` left the tree and `page.md` §2b is what survives it | AM-S4's own cells, plus: a version-17 image holding `kPerCoreStreams` is refused at `Decode` naming the migration; `grep single_stream` over `src/` and `include/` returns only past-tense prose. **Three findings filed rather than fixed**: `page.md`'s decision record still called the pool per-core (AM-S5's H4 grep could not match its wording), the client manual still documented an `extent` lease-refill block AW-S1b deleted, and **`PAGE_HANDOFF` is now written and never read** — `range_alloc.cpp` pays a flush and a device sync per range opening for a record with no consumer, filed in `known-gaps.md` under WAL because retiring a record kind is a format decision | M sized, M delivered |
| AM-S5 | Prose: `eviction.md` §1/EV4 as AM-R7 asks, `page.md` §6, `heap-and-tuple.md` §6, `rules.md` §3's shared-pool row, `crosscore.md` CC7, `CLAUDE.md` | no spec claims a core-local pool. **Built as AW-S1, 2026-09-06.** The list was one file short: `wal.md`'s fuzzy-checkpoint sentence claimed each core checkpoints "its own pools", and a peer's checkpoint target is the shared store (`core_runtime.cpp:954`), so N cores walk one table under one stream — corrected to what the code does, with *how many* checkpointers a shared pool should have left to AM-S3 rather than answered in prose | M |
...
**One defect the review fixed in the stash**: `Expeditor::Open` applies `FrameBudgetShare` at store open and 3c-ii did not touch it, so the instance's only pool came out a factor of `cores` smaller than configured — `configured=100 cores=2` measured `pool_budget=50`, where the two-table arrangement it replaced held 2 x 50. **And CLA's cell was one assertion wide**: of the three added, only the pointer identity discriminates. `frame_budget()` is equal under either arrangement, and `resident_pages()` was read on *core 0's* store on both sides of a peer dispatch, which two tables do not move either. The discriminating form is a peer reading something core 0 has **not** faulted, asserting core 0's count grows. **Step 3 closed at `2663001`, and the seven commits before it are the stage.** `1f464dc` made the claim and the mark one step; `c9b2da4` latched `StampPageLsn` and `MarkFrameDirty`; `0a57fa0` took the collecting readers and split writeback into three phases when the cell written for `Flush` dumped core; `a21fe3f` decided the scan ring - **pin the slots** - which this file's own comment said step 3 had to decide and which two eviction cells then stated better than before (**built and replaced 2026-09-06**: the model held one pin per occupied slot and took no page latch at all, so a scan could read a page a writer on another core was in the middle of, and its `PinForScan` faulted outside the `loading_` set with an eight-attempt retry and a `ResourceExhausted` where every other accessor waits. `workorder-am-s2-p-scan-ring-port.md` is the ruling and the port, from the other session's `c7c3a67`; §1.2-1.5 there is the axis-by-axis record so the alternative is not proposed again. **The branch was retired 2026-09-06** once `raft-marks-2026-09-05.md` had crossed to `main` at `28e2be1`; its history is held by the annotated tag **`archive/am-s2-c7c3a67`** on `272a46e`, which is what keeps the citations to `c7c3a67` from dangling in a fresh clone. Not a version, and not an ancestor of `main`, so `git describe` is untouched); `964c153` gave core 0's store EV3's floor, which every peer had and core 0 never did. The sharing itself then landed with the three side-effects that exist only once a peer borrows, and with the cell cut from three assertions to the one the review measured as discriminating.

**AM-R2's `MayFault` removal moves to AM-S4, and the reason is a schedule fact rather than a change of mind.** AM-R2 is precise - `MayFault` "becomes vacuous under a shared pool and goes", `MayWrite` **stays**, extent leases **stay** - and it is vacuous only where the pool *is* shared. `Expeditor` shares it under `single_stream()`, so a pre-M0 volume still mounts per-core with per-core stores, and there `MayFault` is still the enforcement that keeps a peer from faulting a page another core owns. Deleting it now would change that branch from a refusal into a stale read. AM-S4 is the stage that refuses a pre-M1 volume at mount; the predicate and the fault-grant machinery it feeds (`GrantFaultPages`, and AU-S4's `kRelationFaultGrant`) go there, in the commit that makes the per-core arrangement unreachable rather than merely unusual. **Next is AM-S3**, and sizing it found that **half of it does not exist to be changed**. AM-R6 says to "keep the budget and the truthful error, and count the cross-core case separately"; verified at `dd0bfe9`, nothing reads `kds.evict_retry_budget` and no path returns `ResourceExhausted` for a full pool. EV8's protocol is unbuilt and `eviction.md` §3.3 describes it as though it were - `buffer_pool_frames` is a **soft** target, and a sweep that reclaims nothing lets the insert proceed and the pool grow past it. AM-R6's premise fails the same way: it argues the retry budget "stops being a statement-local fact" under a shared pool, and there is no retry budget. Filed in `docs/inflight/known-gaps.md` under Eviction. **So AM-S3 splits**: the writeback and WAL-gate half is real and largely landed already (the three-phase writeback and `AwaitWalGate`'s scan/wait split at `0a57fa0`), and what remains there is the cell AM-S3 names - flush-before-evict with two cores dirtying one page. AM-R6's counter waits on a stage that builds EV8, which is a piece of work this order never scoped because it assumed the protocol was there. |
| AM-S3 | **Surveyed and then built, 2026-09-07**, and it is an L rather than the M this order sized. **The survey moved the stage.** Its two halves were known to be uneven - the writeback/WAL-gate half largely landed at `0a57fa0`, and AM-R6's counter waits on an EV8 that does not exist - so what was thought to remain was one cell. It was not: **the free map had no concurrency protocol at all**, which is the frame table's AM-S2 step 1 problem in the one structure step 1 did not cover. `map_regions_` was *mutated* outside any hold (`CreateNewUnpinned` drops the latch to call `EnsureRegionResident`, deliberately, because that reads the device and may grow the file - and it ended in a `std::map::emplace`), while `IsAllocated` and `IsHeaderless` reached `find` with no hold at all, from the fault path, the writeback path and the WAL gate. `EnsureRegionResident`'s own comment claimed every caller reached the map under the latch; three of its four call sites did not, and two said so one line away. **The census is how the shape was decided, and it worked by hanging rather than by aborting.** The plan was one latch, per AM-S2's review; the first build guarded the readers with the *frame table's* latch and the suite hung in `PinProtocolTest` - because `FetchPinned`'s hit path holds that latch across `Resolve`, and `Resolve` begins with `IsAllocated`. The detector could not see it: `FetchPinned` takes the latch through `std::unique_lock` rather than `LatchGuard`. So the free map gets **its own latch, inner to the frame table's**, which is AM-S2's review's own *structure -> allocation, allocation the inner leaf* read literally. `base/latch.hpp` gained debug held-latch tracking (`HoldsLatch`) to make that census possible at all, and both halves of the order are asserted rather than stated. **What shipped**: the map latch; `IsAllocated`/`IsHeaderless`/`map_residency` guarded with `...Locked` bodies for callers already inside; `EnsureRegionResident` and `LoadRegionIfPresent` resolving device work outside the hold and publishing with `try_emplace` under it, so a loser keeps the winner's bitmap; `EnsureHeaderlessMap`'s lazy pointer install under the hold with its `EnsureCapacity` moved out; `FlushMaps` as copy-under-the-hold, write-after, returning how many regions it wrote; `maps_dirty()` retired, since it was a second unsynchronised walk to answer a question `FlushMaps` now answers once. **And it answers "who runs the map writeback when one store serves every core" without a rule**: the first core to take the hold clears the dirty flags while it copies, and the rest find nothing. **Two dead things found on the way**: `RecountAllocatedPages` had no callers (its last went with `RefreshFreeMapFromDevice` at AW-S1b), and `maps_dirty` had two, both of which the return value replaces. **What did not land**: AM-R6's counter, which waits on an EV8 that does not exist (already filed), and the flush-before-evict cell this row names with two cores dirtying one page - that is the *frame table's* half of writeback under sharing, not the free map's, and it wants the two-core rig AV owns rather than two threads on one store | `tests/free_map_race_test.cpp`, three cells, **every one of them mutation-measured and the first two shapes of them measured nothing**: dropping `IsAllocated`'s guard fails 7 of 10 runs, publishing the region outside the hold 4 of 8 (one SIGSEGV, and a residency count reading 377 where 13 regions exist), `insert_or_assign` for `try_emplace` 8 of 8, and clearing `dirty` after the write rather than at the copy 5 of 8. The first shape of the creation cell killed **nothing** - 0 of 10 - because four regions created at the start of a 3 ms run leave the readers no insertion to race and its readers all stopped at the first writer's exit; the flusher cell killed 1 of 8 until it got rounds | **L** |
