...
**Status: AM-S0 through AM-S5 are done; AM-S6 has not started and is
...
| AM-S3 | **Surveyed 2026-09-07 at `f054e5e`, not built, and it is an L rather than the M this order sized.** The row's own two halves were already known to be uneven - the writeback/WAL-gate half largely landed at `0a57fa0`, and AM-R6's counter waits on an EV8 that does not exist - so what was thought to remain was one cell. It is not. **The free map has no concurrency protocol at all**, which is the frame table's AM-S2 step 1 problem in the one structure step 1 did not cover: `map_regions_` is *mutated* outside the structure latch (`CreateNewUnpinned` drops the hold to call `EnsureRegionResident`, deliberately, because that reads the device and may grow the file - and it ends in a `std::map::emplace`), while `IsAllocated` and `IsHeaderless` reach `FindRegion`'s `find` with no hold at all, from the fault path, the writeback path and the WAL gate. A `find` racing an `insert` is UB, and AM-S2 step 3c-ii measured the identical shape on the frame table as *"a segfault, not a slow path"*. `EnsureRegionResident`'s own comment claims every caller reaches the map under the latch; three of its four call sites do not, and two say so one line away. Filed in `docs/inflight/bugs/flushmaps-lease-guard-and-unlatched-region-walk.md` §3, which is re-titled because §2's walk is the least dangerous reader in it. **The hazard that makes it an L**: `base/latch.hpp` is not recursive and `IsHeaderless` calls `IsAllocated`, so latching the public readers is a self-deadlock unless they split into `...Locked` bodies - and six more sites (`StampIfHeadered`, `ResidentBytes`, the two writeback loops, `NotAllocated`, `CreateAtUnpinned`'s racy pre-check) have to be censused for whether they are already under a hold, exactly as AM-S1's `KDS_TEST_PAGE_LATCH` census did for the page latch. The shape of the fix is settled and is the one AM-S2's review recommended: one latch, device work resolved outside and re-validated under, `FetchPinned`'s retry idiom. **What AM-S3 also decides**, and the survey did not have to look far for it: with one store and N checkpointers, N cores each run the map writeback today. A latched copy-then-write answers that for free - the first core to take the hold clears the dirty flags and the rest find nothing - which is why the two halves of this stage are one piece of work rather than two | the flush-before-evict cell this row names, with two cores dirtying one page; the region-map race reproduced before it is fixed, the way `alloc_race_test.cpp` reproduced the duplicate-id one; the writeback runs once per checkpoint round rather than N times | **L** |
