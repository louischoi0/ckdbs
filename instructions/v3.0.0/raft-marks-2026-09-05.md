...
| item | mark | obliges |
|---|---|---|
| `ScanRing` under sharing (`device_page_store.cpp:703-712`, "neither is decided") | **pin the one page handed out** — AM-R8, R8a, R8b, R8c | `workorder-am-s2-step3-scan-ring.md` S3a; `eviction.md` §5 sentence (§7 there) |
| R3 — armed/unarmed CLOCK usage disagree after a contended fault | AM-R9: re-check on wake without re-fetching | S3b |
| R6 — hit path's `loading_` test uncovered, fixture budget 0 | AM-R10: budgeted fixture variant; the data-race half stays recorded, not covered | S3b |
| R8 — no `PageDevice` declares thread-safety | AM-R11: the declaration; file-backed device source-read against it | S3b |
...
1. `workorder-am-s2-step3-scan-ring.md` S3a → S3b → S3c (AM-S2 completes).
