...
| AW-S1 | **AM-S5** prose as its row lists: `eviction.md` §1/EV4 (AM-R7), `page.md` §6, `heap-and-tuple.md` §6, `rules.md` §3's shared-pool row, `crosscore.md` CC7, `CLAUDE.md` | H4's grep returns nothing; every struck sentence is replaced by the mechanism, not deleted | M | S0 |
...
| AW-S5 | **AM-S6 — run 2026-09-08 and the stage is complete with its delta not produced** (`bench/v3.0.0/results-am-s6-m1-baseline-v2.7.0-286-g1e7148f.md`; the AM-S6 row in `workorder-am-m1-shared-pool.md` carries the detail). Both gates this row named are met: the host is AL-S8's 8 logical CPUs again, and AN-S2 landed on `main` at `18fed49`. The A/B the operator lifted the suspension for ran 64 times - eight cells, two arms, four alternating passes - and found the two engines the same to within 1.4% at `cores = 1`, and `cores = 8` **unmeasurable on this host**: those cells are bimodal on both arms with 46-80% spreads, so nothing below ~50% is visible there. The lift was worth taking for the one-core half, which is the first end-to-end evidence for AR0's `cores = 1` rule; the eight-core half needs a different instrument, and the results file's §8 lists four in the order CLA would try them, none ordered |
