...
| AO-S3b | **Not started, and it needs two things built rather than a decision made.** A first source read on `worktree-worktree-ar2-borrow-model` at `c168acb` concluded the stage was blocked on an isolation-contract decision; **its own review refuted that, empirically**, and the retraction is the useful part of this row. The claim was that a walk parking mid-statement and re-checking under its own unchanged view must get the same answer, because the view is minted once per statement (`src/txn/manager.cpp:127-142`, `docs/spec/txn.md` §1). **`CheckWriteConflict` is a function of two things, and the argument tracked only one**: the view *and* `cur`, the writer id read out of the tuple header at every check (`txn.md` §5: the verdict is taken "over the *current* header `trx_id`"). An abort's compensation restores `prior_trx_id` as well as the bytes (`src/txn/manager.cpp:409-411`), so after a rollback `cur` is a different id and the same view answers differently — which `tests/txn_manager_test.cpp`'s `RollbackRestoresTheBytesAnOverwriteReplaced` has been asserting all along, and which AO-S3's own comment at `src/server/command_dispatcher.cpp:334-336` already said in words ("an aborted one puts the old version back"). So the stage's cell — a ten-row `UPDATE` meeting a held row 7 keeps rows 1–6 and **waits** — is deliverable with no contract change: **the abort arm completes all ten rows under one snapshot**, and the commit arm is a `TxnConflict` that `txn.md` §5 already ratifies as deliberate ("stricter than PostgreSQL's `READ COMMITTED`… a deliberate simplification"). Nothing here is open. **What is actually owed, and none of it is an operator's to answer**: (1) **a waiter identity for an in-flight autocommit scope.** AO-S4a's graph exempts autocommit because `EndWrite` unwinds an owned scope *before* the wait loop runs, so such a waiter holds nothing and `waiter_id` is legitimately 0 (`command_dispatcher.cpp:311-319`). A mid-statement park breaks that: the scope stays open holding rows 1–6, a second transaction can block on row 3, and the cycle is real while **no edge is registered** — invisible to the detector, ended only by the 11 s net, which kills both waiters and is what AR2-R10 forbids. (2) **`check_view` must be re-minted on resume.** It is minted once per statement at `command_dispatcher.cpp:9893` and consumed per row; across a park it is stale, and `docs/spec/foreign-keys.md` §4 promises the forward check reads **latest state**. (3) The page loop hoisted out of `ChainVisit`/`BtreeVisit` into the coroutine, `src/exec/step_vm.cpp:1943-2007`'s shape, with the five catalog borrows re-taken after each park where the step VM re-takes one. **Two supporting facts from the same read, both verified**: AO-R5's stated hazard for refusing a restart — "re-applying `SET v = v + 1` would be a second increment" — cannot occur, because `parser::Assignment` carries an `AstValue` and `ParseUpdate` calls `ParseValue()`, so no expression is representable; and a resume cursor is expressible, though as `(range, page, ordinal)` rather than by slot — `exec::WalkMark` (`step_vm.cpp:186-199`) chose the ordinal deliberately, "the emission order is the page's, and it is not always slot order", and its safety argument ("nothing writes to the relation between the outer rows") is exactly what a park breaks, so a resumed walk owes its own argument. `(page_id, slot)` is stable for a row's life **until relayout moves it**, which is shadow-only today |
