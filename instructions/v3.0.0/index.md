...
| `workorder-ao-m2-lock-family.md` | **AR0 M2 — the lock family**, the stated M2 AR2-A §6 asked for so AM and AN proceed against it. Written 2026-09-03 at `9e5068c`: the refusal census (13 rows, each classed by locality and each turned into a wait, retired, or kept), the survey of the wait substrate (`WaitUntil` is a poll loop; a same-core grant needs no wake; the in-doubt block is a working prototype of a tuple-lock wait that ends by clock), rulings AO-R1..R14 — among them **no persisted lock bit** (the `trx_id` stamp is the tuple `X` fast path; the Keystone byte stays zero, decided by the operator in plan mode) and D12's detector as the mechanism with a 1 s fault net — and stages AO-S0..S8 with S1–S4a and S3b gated on the operator's word alone and S5 onward on AM-S1..S6 and AN-S2. **AO-S0 landed with this row**: the document and `rules.md` §3's lock-table row, marked not built |
