...
| remote steps | `kStep{Open,Batch,Eof,Credit,Cancel,Error}` | **struck** — AT-0 item 4, answered at AT-S9: no engine producer since that stage; AT-S10/S11 delete them |
...
must list, not one it may strike"* — so AT-S10 builds it and AU-S6's count
(AT-S10d; AT-S11 struck) must reach 0 with it counted, not by ignoring it.
...
| **AT-S7** (built) | **Statistics and Cabin local.** `AccessBatch` fold-and-flush becomes a local write under lock (CC13), `kAccessStatsBatch` struck; the two `CabinStore`s (AT-3 G) become one store under whichever of AR1 §11's two shapes AT-0 item 9 names; `cabin.md` §4b's scope rule struck; Waystone's peer-recording default lifted | `SHOW ACCESS` totals match the pre-change engine for the same workload. A peer's Cabin observation is banked and served. The contract suites for waystone and cabin stay byte-identical across configurations | M | S5 |
| **AT-S8** (built) | **Checkpoint and the listener.** `RemoteCheckpointAnchor` retires; `Checkpoint()` becomes an instance task under an at-most-one-running flag; `kAnchorWrite` struck. D19's listener half as AT-R12 scopes it: `peer_listeners` becomes the arrangement, and its TLS/SCRAM refusal is removed with the core-local credential store or handed on by name | two cores cannot run a checkpoint at once. A session accepted on any core runs to completion there. TLS with per-core listeners, or the row says whose it is | M | S5 |
| **AT-S9** (built) | **Placement and `owner_core`** (D17, D18, E8's verb, E9's withdrawal). AT's format event drops both columns and moves AT-R7's four anchors, adding the `SysTableRow` assert that is missing; `core_placement.hpp` and `PlacementPolicy` retire; NS10's verb becomes "declares the affinity of"; `core_count`'s pinning is decided with the rest of core specialization | a pre-AT volume mounts and its `owner_core` values are ignored — no refusal. `ns.table` still resolves; namespaces still exist as names | M | S5–S8 |
...
| AT-S5d | **built 2026-09-10** on `m3-at`: one `AssertionEnforcer` for the instance (D1, AT-R15). **The registry** - `Expeditor` owns it, armed at `cores > 1`; `CoreRuntime::Config::assertions` hands it to every peer; a dispatcher never handed one builds its own, `LockTable`'s idiom. **Core 0's mount resumes it for every relation** and a peer handed it resumes nothing - `ResumeAssertionsAfterRecovery` loses its `owner_core` filter, and `MountRecovery::assertions_foreign` goes with `SHOW META`'s `recovery_assertions_foreign`, a counter nothing could move. **Two latches, both null unarmed** (`assertion_check.hpp`, `rules.md` §3's new row, `sched.md` §9-2): the ruled **directory latch**, which never spans page work, and a **chain latch** per assertion, which does - the ruling named one latch and the stage needed two, because a cabin chain's tail and growth are one writer's state and the growth is page work. **The admission holds** what it admitted (`AssertionEnforcer::Hold`): a hold counts in every later admission and in no header, `ReserveInsert` converts it one step with its `ASSERT_RESERVE`, and every exit that does not reach the reservation gives it back - a refusal, a failed placement, a borrow that parks and re-runs. A negative contribution is held as zero, and an `UPDATE` holds its arrival's **full** contribution whatever it checked, because its departure lands first and would otherwise read as room to another core in between. An assertion adopted after a row's admission is admitted at its reservation instead. **What the ruling did not name, and the stage found by reading the recovery pass**: a checkpoint's `ASSERT_SNAPSHOT` is a base only if it equals the fold of every record before it, and with a reserver on another thread a snapshot could land between a header change and its record. So the directory latch also spans the WAL append that describes a header change, and the snapshot is taken and appended under it through `AssertionSnapshotSource::VisitSnapshots`, the seam's one method now; `BoundCabinChainWriter::Append` splits into `Place` and `Log` so the record can be appended inside the latch while the page work stays outside it. **The ship is retired** - AT-7 item 13's `:4020` and `:4047` branches: `CREATE ASSERTION` builds where its session is and adopts into the registry, `DROP` evicts from it, `server/assertion_build_service.{hpp,cpp}` are deleted, ring kinds 30-32 are struck and D25's freeze moves 29 → 26, and the success line's `built_by_core=` and `SHOW ASSERTIONS`' `enforced_by_core=` go with it. **Found and not closed**: the build takes no relation lock, so a writer on another core can land a row the scan already passed before the directory is adopted - a quiet understatement that predates this stage (the owner's build had the same window since AT-S5). `docs/inflight/bugs/create-assertion-build-is-not-fenced-against-writers.md` and AT-0 item 13; `assertion.md` §8.1 now says so instead of *"no write can interleave with the build"*. Cells: five in the new `tests/assertion_race_test.cpp`, on real threads over an armed store - two cores admitting into one group admit exactly one **and refuse the other at its admission**, an assertion adopted between admission and reservation is checked there, a hold counts in every admission and in no snapshot and is room again once given back, a snapshot holds the directory against a reservation on another core, and eight cores reserving into one cabin lose no entry. **Mutations, measured**: the admission taking no hold, killed 5 in 5; the reservation skipping its uncovered check, 1 in 1; the snapshot without the latch, 3 in 3; a hold never given back, 1 in 1; the directory latch never armed, 5 in 5 against the eight-core cell. **The chain latch removed survived 10 in 10, at two writers and again at eight**, and the cell says why rather than claiming it: a growth links from the *live* tail under that page's latch, so losing a link takes a third writer inside another's growth, a window the shape does not open. The latch stays for the plain fields two threads would otherwise write, and no cell pins it. Two cells in `core_runtime_test.cpp`: `UnderOneStreamAPeerWithAnAssertionInTheCatalogStillMounts` re-pointed from `assertions_foreign` to the unrevivable declaration refusing its relation's writes, and `APeerHandedTheInstancesAssertionRegistryResumesNothing` new. **`ExpeditorTest.APeerThatOwnsAnAssertionMountsAndComesUpEnforcingIt` passes, and it was edited to pass**: its enforcement reading - a second row in group 7 refused on core 0's listener for a relation `kRotate` placed on core 1, which is D1 exactly - is unchanged, and its counter reading moved from the peer's recovery report to core 0's, because the resume moved; it now also pins that the two cores hold one registry. Three `core_runtime_test.cpp` cells retired in AT-S5's one-line form - the owner's restart folding its own cabin, a refused foreign build, an abandoned one - and the four earlier records that said the handlers stayed for AT-S10 now say they went here. Specs: `assertion.md` §6 retitled and §6.1 rewritten whole, §6.2's steps and two properties, §4.2, §4.3, §7's AS6a note, §8.1, §8.1a and AS4's row; `crosscore.md` CC7's owner-builds exception for assertions and §6a's gate, whose two of three reasons are gone and which is kept unre-decided; `client-manual.md`'s recovery block; `manual/sql/sql.md`'s multi-core bullet; `CLAUDE.md`'s Assertions row. `docs/inflight/bugs/assertion-enforced-only-where-the-directory-is.md` deleted, the bucket's rule for an entry its stage closes. **Suite 3360/3360 in 70.86 s** on the tree committed - AT-S5c's 3357, less three retired, plus six new, and D1's failure gone. **Its `critics-developer` pass** (on `3d58041`, applied in the next commit) found the locking and the hold accounting correct and **two defects**, both fixed. **D-1, new with the stage**: every core's checkpointer snapshotted the one registry, and each `CHECKPOINT_BEGIN` is appended outside its latch, so two cores' runs could land back to back and recovery would meet the second run's first group with the first run's base still open - a duplicate group id, a failed pass, every asserted relation refusing writes for the mount. **Only core 0 snapshots the instance's registry now**; a mount's scan starts at or below core 0's own `CHECKPOINT_BEGIN`, so its snapshot is always in range, and the redundant snapshots go with it. **D-2, reachable since the stage**: an `UPDATE` asked `AnyOn` twice per row, and an adoption between the asks sent an empty `previous` to the check - a read past its end; asked once now. **Three should-fix items, two applied and one declined.** Applied: the durability claim had no cell - every race cell ran without a log - and now has one on a real shared stream; and the registry's latch sat on every write's path for relations with no assertion at all, which a count read without the latch (`NothingDeclared`) now skips, published under the latch by the three calls that change it. **Declined**: that a waiter named by a *hold* can wait on a transaction whose hold was then given back. It is the right transaction to wait on - a statement that fails between its hold and its reservation inside an explicit transaction poisons it, so its decide follows the client's `ROLLBACK`, and one that parks re-holds on its re-run - and the wait is bounded by the fault net; naming reservations only would put back the false rejection AO-S6e-c removed. The operator may overrule it. **The cuts**, all applied but one: the build's split is internal to `assertion_catalog.cpp` (its second caller is gone; the build still re-resolves the relation, because resolving once changes the build's inputs); the per-group hold sums are gone and an admission sums `holds_` by a scan, bounded by the rows in flight; one `AdmitLocked` replaces three copies of check-count-refuse-hold; `AdmitInsert` takes its `Hold` by reference, so no write path can admit without one; `AssertionSnapshotSource` has one method; a handed registry frees the dispatcher's own. **Declined**: moving the build's base snapshot after the publish, a durability change of its own. Stale prose fixed in `ddl-transactional.md` (the preamble, §5's list and §5f rewritten), `client-manual.md`, `assertion_catalog.{hpp,cpp}`, `range_eligible.hpp`, `assertion_check.hpp` and four dispatcher comments. **Found beside it, pre-existing since AR0 M0**: recovery closes a chunked snapshot's base at the first non-snapshot record, and another core's record can land between two chunks - `docs/inflight/bugs/a-chunked-assertion-snapshot-can-be-split-by-another-cores-record.md`. The unfenced-build entry now names the `UPDATE`, `DELETE` and sorted-fill paths too. Three cells added - a held negative contribution lends no room, every snapshot equals the fold of the records before it on a shared log, and a peer handed the registry writes no snapshot of it - and **every mutant of the second round killed**: the admission giving its hold straight back 5/5, the uncovered check skipped 1/1, the snapshot without the latch 3/3, negative holds counted 1/1, the record appended before the latch 5/5, a hold never given back 1/1, a peer snapshotting the registry 1/1. **Not covered by any cell**: an `UPDATE` holding its arrival in full, whose window is inside one call. **Suite 3363/3363 in 71.78 s** on the tree committed with the review, the stage's 3360 plus three. Overhead not measured |
| AT-S5e | **built 2026-09-11** on `m3-at`, with AT-0 item 13 on the operator's word: **`CREATE INDEX` and `DROP INDEX` take the relation `X`** (D6, AT-R15), and **a `CREATE ASSERTION`'s build takes it too** (item 13). The index statements take it through `BorrowRelationForDdl`, `DROP TABLE`'s borrow, before their first catalog write and on whichever core the session is; the build takes it under **a transaction of the statement's own** that writes nothing and is rolled back at the statement's end, so the relation is released there and a failed `CREATE ASSERTION` still poisons nothing - item 13's proposal said "a non-transactional holder id", and it is a transaction rather than a minted id because a waiter names its holder and a minted id reads as "a positioned reader". The one holder the build cannot wait for, the session's own transaction having written the relation, is answered at once with the build's in-flight refusal. **The writer's side, which the ruling did not name and the stage could not do without**: a writer's relation `IX` refused by a DDL used to fall to the row-level wait, which polls `IsInFlight` - one core's live set - so a writer on core 1 would have read a `CREATE INDEX` on core 0 as finished and been refused. `BorrowChain` now registers a wake on a refused relation `IX` and the statement parks on the table's own slot, `DROP TABLE`'s wait from the other side; `NoteBlockingWriter` stands down when one is set. **An `INSERT` asks its relation `IX` in `InsertParsed`**, ahead of its assertion admission and the sorted fill's `AnyOn` gate, where it used to take it at its first id borrow after both - item 13's other half; an `UPDATE` or `DELETE` takes it at its declared borrow or, where its predicate declares no key window, at its first qualifying row (the review corrected the first draft, which said "ahead of its walk" for both). **Retired with the window**: AO-S6e-a's wait (`AwaitIndexWindow`, `index_window`), `IndexBuildPending`, `PendingIndexBuilds`, `kIndexWindowWaitNs`, `SHOW META`'s `index_build_windows` and `index_build_window_age_max_us`, and the `DROP INDEX`-in-a-transaction refusal on a peer-owned relation (the first draft said DT9's predicate is then never asked about a writer; the review showed it is, and what makes it harmless is below). **Retired with the ship** - AT-7 item 13's `:3277` and `:3310`: `BeginForeignIndexBuild`/`FinishIndexBuild`, `server/index_build_service.{hpp,cpp}`, ring kinds 25-27 (D25: 26 → 23), the reply's `built_by_core=`, and `Catalog::AnchorSeed` with its `kByOwner`, which had no caller left. **Left for a decision it bears on**: the catalog's publish-hook gate would now refuse the ruled build if a publisher were installed; `docs/inflight/bugs/publish-hook-gate-is-test-only.md` is re-verified and says only its options 2 and 3 fit AT-S5e. Cells: three in the new `tests/ddl_fence_rig_test.cpp` on the two-core rig - **an INSERT on core 1 waits for a CREATE INDEX on core 0 and its row is in the index** (the done-condition; the build held open by an explicit transaction), a `CREATE ASSERTION` on core 0 waits for a writer on core 1 and counts its row, and a writer parks on a relation `X` before its assertion admission (the check counter reads 0 while it waits) - and one in `LockDeadlockTest`, a build over its own transaction's write refused at once. The rig takes the instance's assertion registry, as `Expeditor` hands one. **Mutations, measured, all killed**: the index DDL taking no `X` 1/1, a refused relation `IX` registering no wake 1/1, the build taking no `X` 1/1, the `INSERT`'s `IX` left after its admission 1/1, the own-transaction test removed 1/1. Eight `core_runtime_test.cpp` cells retired in AT-S5's one-line form - the window's wait, the owner build and publish, its refusal in a transaction, its deadline, the owner's second-build refusal, an abandoned reply, the synchronous path's abandonment, the owner's raw-payload endings - and three flipped, each renamed for what it now says: a `CREATE INDEX` on a peer-owned relation builds where it runs, a peer maintains an index core 0 built, and a `DROP INDEX` on a peer relation is admitted inside a transaction and rolls back whole. `coro_test.cpp`'s stand-in kinds are `kShippedStatement{Request,Reply}`. Specs: `ddl-transactional.md` §5e rewritten whole, its preamble, §5's list, §5b's scope paragraph and §5f's isolation paragraph; `assertion.md` §6.1 and §8.1; `txn.md` §5's consumers of the relation `X`; `crosscore.md` CC7 and §6's counter note; `sched.md`'s wait table; `manual/sql/sql.md`; `CLAUDE.md`'s Transactional DDL and Assertions rows; stale citations of the retired client and server in five service headers. `docs/inflight/bugs/index-build-window-is-invisible-to-the-core-that-writes.md` and `create-assertion-build-is-not-fenced-against-writers.md` deleted, the bucket's rule for entries a stage closes. **Suite 3359/3359 in 65.83 s** on the tree committed - AT-S5d's 3363, less eight retired, plus four. **Its `critics-developer` pass** (on `d014aa2`, applied in the next commit) found **five defects** and all five are fixed. **C1**: a `DROP INDEX` rolled back on core 0 released its `X` before the DDL path bumped the schema word, and a peer's `INSERT` - which had resolved the relation while the drop was open and left the index out, DT9's predicate being core-local - re-ran in that gap with its memo and wrote no entry into the index the rollback kept. A transaction that wrote catalog rows now carries `NoteWroteCatalog`, and the manager moves the word **before** it releases the borrows, at commit and at abort. **C2**: a writer's memo is settled at its task boundary and its first `IX` comes later, so a `CREATE INDEX` could take the relation, publish and release in between; a writer's **first** relation intention now checks `Catalog::MemoIsCurrent` and, moved, fails the statement re-runnably (a pre-flipped slot, holder 0, no edge). **C3**: a foreign-key probe's resume waited for a row but not for a relation, so a relation refusal reached the client and leaked its wake; one `AwaitStatementWaits` now serves both callers. **C4**: a busy parent recorded before a relation refusal installed both waits, and the write-block re-run dropped the relation's wake; the row blocker is not installed beside a relation wait. **C5**: the build's own transaction was aborted and never released, so every `CREATE ASSERTION` left an entry in the manager's list; `Release` follows `Abort`. **Should-fix, all four applied**: a parked build's wait could poison the session at the fault net or as a deadlock victim - `LockWait::poisons`, false for the build (C6); the own-transaction test trusted the one blocker the table reported and is now asked of the session's holdings (C7, `LockHoldings::Holds`); `Commit` lifted the pending-commit marker after releasing the borrows, so a build woken by the release could mint a view below the commit - the marker lifts first (C8); the build's name is resolved under the session's view (C9). **Simplifications taken**: the wait loop in one function, the build reusing `BorrowRelationForDdl` (which takes a holder and the poison flag now), one `RelationHeld` spelling, `CreatedIndexReply` without its dead parameter, retirement history trimmed from new comments. **Declined**: `BorrowChain`'s `blocker` as a reference - three call sites for no behaviour. **Stale text fixed**: `ddl-transactional.md`'s preamble, §5b, §5e's fence paragraph; `LockWait`'s comment, which still said `DROP TABLE` was the one asker and no edge was drawn; `read_borrow.hpp`; `ring_message.hpp`; the dispatcher's history comments; `manual/sql/sql.md`'s index section and `CLAUDE.md`'s two rows now say the statements wait and can be refused at the 11 s net. **Not changed**: `workorder-ax-inflight-publication.md` still cites the core-0-scoped DT9 clause this stage removed from §5b - another work order's text, left for its owner. Three cells added - a `DROP INDEX` rolled back on core 0 keeps the row a peer wrote while it was open (eight rounds on one rig, because the unfixed path is a race), an `UPDATE` on a peer waits for a `CREATE INDEX` and maintains it (the per-row path), and a `CREATE ASSERTION` leaves no transaction behind. **Mutations, measured**: the pre-release word move removed 5/5 (1/3 at one round, which is why the cell loops), the build's `Release` removed 1/1, a relation `IX` with no wake against the `UPDATE` cell 1/1. **Uncovered, and said**: C2's stale-memo check survived its mutant 0/1 - no cell holds a writer between its boundary and its bind, which is where the window is - and C3, C4, C6 and C8 have no cell of their own. **Suite 3362/3362 in 66.39 s** on the tree committed with the review, the stage's 3359 plus three. Overhead not measured |
| AT-S5f | **built 2026-09-23** on `at-s5f-fk-local`: the foreign-key checks go local (D5 and D4's walk half, AT-R15). **The forward check stops deferring**: `ResolveForeignKeyParents`' `owner_core` arm is gone, so every parent is descended where the statement runs - the descent was always ownership-free and the deferral above it was the only thing that made a parent foreign, a page every core has faulted since AM-S2 step 3. The hoist stays and its reason is restated rather than inherited: deduplication (AH-R2), and a wait that has to happen before any row is written. **The reverse check walks every chain** - `TableAccess::AllWalkHeads`, its own method rather than a sentinel on `WalkHeadsFor`, because *walk what you own* is the read path's rule and a constraint check has no second stage behind it - and its scope refusal goes with the fan-out it stood in for. **An in-flight parent is a wait on any core**: `NoteBlockingWriter` polls `IsInFlight`, which is one core's live set, so the busy arm asks the instance's table for the parent row's own entry and parks on the slot the holder's release flips from whichever core releases (AO-S6e-b's idiom, reached from the other side). **A refused `TryAcquire` acquires nothing** - no queue position, nothing in the holdings, only the wake registration - which is what keeps this a wait and not D9(a)'s fence; a *granted* ask means the holder decided between the header read and the ask, releases at once, and takes the busy verdict's refusal, which is what a decided holder always produced. **A per-core Cabin set may find a child and may not clear one**: the drained loop falls through to the walk until AT-S7 makes the store the instance's (AT-0 item 9). **Retired with the protocol**: `FkIntentTable`, `FkPendingDeleteTable`, `fk_probe_service.{hpp,cpp}`, both senders, the probe park in `DispatchAsync` and AK-S3's rounds loop, the collect pass and `FkDeleteRows`, `resumed_from_fk_probe_` and `AwaitStatementWaits`' `resumed` parameter, `EndWrite`'s `statement_ends` and `InsertParsed`'s `line` - each had exactly one reader and it was the park - the session's intent holders, `IntentOnlyTargets`, `DecideTargets` and `last_txn_id`, the 2PC wire's `intent_only` byte and the `transaction_id == 0` decide it admitted (AO-S5(b) C2's arm, which had nothing left to send), five `SHOW META` counters and the `fk_probe_round`/`fk_release_decide` legs. Ring kinds 41-44 struck; **D25's freeze 23 → 19**. Two unused parameters found beside them and removed: `CheckWriteAffinity`'s `relation`, unused since AT-S5 retired the refusals that named it, and `SortedFillInner`'s `line`. **Cells**: the two-core wait cells re-pointed at the lock slot and the file renamed `fk_cross_core_rig_test.cpp` because `FkProbeRigTest` named a protocol that is gone; a new cell on the same rig for D4 - a drained Cabin set on core 0 and the child written on core 1; the scope refusal's two cells turned into the answers they now get; the synchronous-path `DELETE` that was refused retryably and now runs. **Nine cells retired**, each with the premise that went: the probe park's abandon cell, the three item-16 resume-meets-a-fence cells and the mid-walk one (there is no resume), AK-S3's rounds and collect cells, the intent decide and the intent busy, `ForeignParentsGroupByOwnerAndDeduplicate`, and AJ-T1's six pending-delete clears. **Mutants**: the wait not taken 5/5, the Cabin `kPass` restored 5/5, the scope refusal restored killed by both cells. **Two of the first three mutants survived and the cells were wrong, not the mutants** - the finding worth the row: `trades_h` is a **btree** relation since SUS-1 made BTREE the storage default, so the heap arm's heads were never walked and a single dispatcher's Cabin store sees every write, so the drained return is unreachable from one core. The first replacement cells were rewritten on that reading rather than kept. Specs: `foreign-keys.md` §2a rewritten whole, §2b is now the record of what the crossing was, §3a rewritten, §3's Cabin bullet and §5's two bullets; `crosscore.md`'s FK bullet, `cross-owner-txn.md`'s intent-holder paragraph, `txn.md` §5's two mentions, `namespace.md` NS8's pricing sentence, `manual/sql/sql.md`'s namespace and foreign-key sections (a parent being written is **waited for**, which the manual said was a refusal), and `CLAUDE.md`'s row. Both `docs/inflight/bugs/` entries deleted and `known-gaps.md`'s Foreign keys section emptied: AO-S5(b) C1's asymmetry needed a shipped `DELETE` (gone at AT-S5) and the pending-delete pre-gate (gone here). **The stage's second done-condition could not be met as written**: *"the deadlock rig's cross-core FK cycle still refused"* names a cell `deadlock_rig_test.cpp` no longer has - AT-S5 retired it, because a cycle through two shipped statements needs two shipped *writes* and a write runs where the session is. The property it named holds by construction: the FK wait is recorded as `lock_wait_` and taken by `AwaitRelationLock`, which registers `waiter -> holder` and makes a closer the victim (AO-R7) before it parks. **No cell drives a cycle through the foreign-key check**, and that is an absence rather than a claim.  **Its `critics-developer` pass found the two cells still wrong and one claim of the stage's false.** The cells: `trades_h` names no storage word, so SUS-1's BTREE default made it a **btree** child whose reverse check takes `BtreeVisit` and reads no directory - both range cells passed without reaching `AllWalkHeads` at all, and the mutant that walks this core's heads alone survived them. The word `HEAP` is all it took, because the test binary lifts the suspension (`HeapStorageAllowedForTest`) and only the *parent* must be btree - so the row's own "not covered by any cell" was wrong twice over, about the coverage and about why. Mutant killed after the fix. **Four defects it fixed**: a wake registration leaked whenever two asks parked in one statement - the forward check's at the fork, then `BorrowChain`'s overwriting `lock_wait_` with no `DropWake`, an entry and a dead waiter for the life of the instance (`TakeLockWait` now drops the outgoing one first); `BorrowOrWait` read *any* `lock_wait_` as "the relation refused this ask" and so named the relation where a row or a range was held, skipping the `NoteBlockingWriter` a narrower refusal owes (it tests the unit the wait names now); the struck kinds 41-44 left no marker, where AU-R4 obliges one and every earlier strike has one; and `TxnDecideRequestPayload` lost `reserved0` with the byte, leaving the claim "the space stays reserved" to implicit padding. **Neither A2 nor A3 has a cell** - both need an interleaving the two-core rig could reach and does not - and the review says so rather than implying coverage. **And it found the window this stage opens** (below), which the first draft of `foreign-keys.md` §3a denied in as many words. **One finding declined**: `FkReverseOptions` is two fields and one caller and could be two parameters, but `CheckNoChildReferences` already takes seven and two more positional pointers at the call site is the trade going the wrong way. **The client-visible notice it flagged is fixed**: `CREATE TABLE` told a user that a cross-owner foreign key costs "one cross-core probe round" per write and that "a DELETE of a referenced parent row is refused", and a cell asserted both strings - green while pinning a falsehood. Both costs died in this stage and no third replaces them, so the notice and its two assertions are deleted rather than reworded; the cell now pins the silence and that the constraint is declared. **What this stage opens, and it is the row's most important line**: a forward check that **passes** takes no borrow, so between the check and the child's row write nothing holds the parent still - and across cores a `DELETE` of that parent fits in the gap, walks a child that has no row yet, marks and commits. A committed child referencing a deleted parent, both statements reporting success, which is §1's one forbidden answer. One core closes it by running to completion between the fork and the write; two reactors do not. **The reference intent closed it and AT-R15 retired the intent**, so the window is what the ruling entails rather than a slip in carrying it out - and **D9(a)'s `S` fence closes it exactly**, because the parent's `DELETE` already takes that row's `X` before it walks. Carried as the one entry under `known-gaps.md`'s Foreign keys, owned by the following letter (AT-0 item 6), and stated in `foreign-keys.md` §3a. **Suite 3312/3312** (Debug) on the tree committed with the review's fixes. Overhead not measured |
| AT-S6 | **built 2026-09-23** on `at-s6-2pc-retired`: the cross-owner transaction and 2PC retired (AT-R6) - and **the stage's survey is what made it buildable**, because AT-R6's premise was false. *Once a write runs where the session is, a transaction has no participant on another core*: `ShipStatement` had one caller left, `HandleSelect`'s fork, and it enrolled a participant whenever the session was in a transaction, so every 2PC round the engine still ran was a read-only one. **The survey also found a live defect**: the two halves of one transaction ran under two ids, so a transaction that wrote a peer-owned relation locally (AT-S5) and shipped a read of it back saw zero rows - measured on the two-core rig, `INSERTED`, then a header and no rows, then `41` after `COMMIT`. The fork's own comment argued for shipping on exactly that ground, which had become the argument for the bug. Landed as an entry and a `DISABLED_` cell, and **the operator took AT-0 item 4 / D18's first shape the same day**: a read stops shipping. **What that settled before a line was written**: SA-T0 skips the record for a participant that wrote nothing and AT-S5 left none that could write, so no `TXN_PREPARE` had been appendable since that stage - the stage's first two done-conditions held already, and the third (*the mount scan's own cell states what replaced it*) **is not met**: `prepared_recovery_test.cpp` still synthesises the record and carries no such statement. **The read**: `CheckReadAffinity`'s owner arm and the ship below it go, and `ServableBy` loses its `owner_core` conjunct - the question is whether one walk here answers the relation whole, and ownership stopped bearing on it when AM-S2 step 3 made the pool the instance's. The review verified the re-scope is a no-op for the three other callers, because `OpenRangeRows` always writes the `lo = 0` anchor with the relation's own owner. A split relation keeps the fan-in and the refusal for shapes it cannot take. **The Cabin is held where it was**: making the read local brought peer-owned relations to this core's store for the first time, so `CabinScopeCovers` asks whether this core owns the relation - which also gates the recording path, so no sighting accrues on a peer. That exposure is its own defect and its own entry, found by the same survey and reproduced: `SELECT id FROM r0 WHERE v = 7` answers `id\n17` where `COUNT(*)` says 2, because a set is banked from a walk and kept in the **writing** core's store. AT-S7's. **Retired**: three services and four test files, the fork, the park, the commit protocol, the rollback's participant leg, the enrolment and the participants, every wiring site, both `SHOW META` blocks. Ring kinds 28, 29 and 33-38 struck; **D25's freeze 19 -> 11**. **AO-R8's net falls 11 s -> 1 s**, which has its stated precondition here and nowhere else. **Cells**: twelve retired with the premise that went, four re-pointed at what now happens. **Its `critics-developer` pass found the stage's one operator-visible defect**: `lock_wait_fault_net_ms` - AT-0 item 7's re-scope of `in_doubt_ceiling_ms` - was parsed, range-checked and threaded to every dispatcher while the four wait sites read the `constexpr`, so the key did nothing, `= 0` did not refuse at once as three documents promised, and the old spelling was refused at startup naming an inert successor. Wired: the sites read the member, its default is the constant, and **`CoreRuntime::Config`'s field is defaulted to the constant too** - the first build left it at 0, which is a value an operator may set rather than a sentinel, and turned every unconfigured peer into "refuse at once", breaking eight rig waits. `TheFaultNetsKeyReachesTheDispatcherThatWaits` is the cell. The pass also fixed an empty task submitted on every drain tick and a `set but not used` warning the stage reported a pass over, deleted ~250 lines of write-only state (`session.hpp`'s shipping identity and participants, `commit_phase_stats.hpp`, `utf8_prefix.hpp`, `AbandonWriteForShipping`, `CrossCoreReadNotImplemented`), and corrected a dozen comments arguing for code that is gone - among them one that would have made this stage's premise false, and thirteen citations to deleted headers. **A consequence the stage records rather than fixes**: AN-R14's lifetime ceiling was enforced over participant contexts alone, and that sweep went with the executor - so no transaction has a ceiling, and an abandoned explicit transaction holds the read horizon for the life of the process. `known-gaps.md` carries it; `drop-table.md` DT7 carries the net's fall making its refusal eleven times easier to reach. Specs: `cross-owner-txn.md` is its own retirement record, `crosscore.md` §4a/§5/§6, `client-manual.md`, `txn.md`, `wal.md`, `namespace.md`, `manual/server/server.md`, `kds.conf.sample` and `CLAUDE.md`. **Suite 3120/3120** (Debug), warning-free. Overhead not measured |
| AT-S7 | **built 2026-09-23** on `at-s7-one-cabin-store`: statistics and Cabin local. **One `CabinStore` for the instance** (AT-0 item 9, AR1 §11's second shape on the operator's word): partitioned 16 ways by `cabin_id`, one latch each, and `Find` hands out a **`CabinSet` handle** - a `shared_ptr` to the set plus a count fixed at the take - where it used to hand out a `std::vector<CabinEntry>*` into its own table, which was sound only while the store was core-local. `CoreRuntime::Config::cabins_store` borrows it and `Expeditor` hands it to every peer; a runtime builds its own only where nobody handed one, which is a fixture. **AT-S6's owner test in `CabinScopeCovers` goes with the defect it held**, and `docs/inflight/bugs/a-cabin-set-serves-a-query-short-across-cores.md` is deleted. **Enabling that bug's cell found a second thing**: it compared against a real newline while a debug-text response escapes its rows as `\n`, so the assertion could not have matched whatever the engine answered - it compares the whole reply now and asserts the store's `hits` moved, without which the row assertion passes on a walk. **Access statistics are written where the statement ran**: CR7's `AccessBatch` fold, `access_stats_service.hpp`, `MaybeFlushAccessStats`, `Expeditor`'s applied counters and handler, the recorder's sink parameter and `SHOW META`'s six-field block all retire, **ring kind 39 struck, D25's freeze 11 → 10**, and `CoreRuntime::Open`'s hard `/*access_statistics=*/false` becomes the instance's switch. `RecordAccess` holds `sys.access_stats`' root page exclusive across its body - not for the increment, which `ForFirstRow` already makes atomic, but for the **admission**: two cores that both miss one new shape both admit it, and `ForFirstRow` stops at the first match, so the count splits and stays split. **`cabin.md` §4b's scope rule is struck** with the per-core store whose write hook was its whole premise; what survives is not the set's authority but the *step*'s span, which may neither bank from nor answer from a walk narrower than the relation. **And striking it exposed that §6's soundness argument was already false.** §6 deleted the build-by-observation hazard structurally - a statement runs to completion on its owning core - and said in as many words that if that model changed, §4-§5 must be redesigned rather than relaxed; AT-S5 moved the write, AT-S6 the read, this stage the store. The redesign is the **announce**: `BeginRecording` installs an empty set `Find` refuses to serve and `NoteWrite` appends into, so every write on every core during the walk is in it before the walk's matches merge in, and `Unobserve`, the entry cap and `Discard` poison an in-flight announce rather than erasing it. What the announce cannot cover is the gate's, and the gate is the instance's now: `in_flight_at_mint` asks `InstanceVisibility::AnyUnresolved` as well as its own `live_`, and after the announce the gate asks again, the announce's own partition latch being what orders those loads against writes the hook has taken. **Waystone's peer-recording default is lifted**, and two persisted races closed first - `RegisterPattern` holds `sys.patterns`' root page, and `SetPatternWaystoneRoot` becomes `ClaimPatternWaystoneRoot`, a claim that keeps the first directory and tells the loser which pair won rather than stranding every trail in it. **Cells**: four on the announced build and two on the heal's write-back in `cabin_store_test.cpp`; `TwoCoresRecordingOneShapeShareItsRow`; `ASecondDirectoryIsRefusedAndTheFirstIsAnswered`; and three on the rig - `APeersStatementIsCountedInTheOneRelation`, `APeersRepeatedStatementRegistersItsPattern` and `ASetBankedByAPeerServesThisCoresQuery`, each with its mutation run and scoped to the peer, because both cores are `CoreRuntime`s on the rig and an unscoped mutation fails a cell at its premise instead of its claim. The waystone cell also records what it took to pass: **a peer's first pattern registration waits for a `sys.patterns` row-id grant**, the same demand-then-grant its first INSERT pays. `access_batch_test.cpp` retires with its two CR6/CB8 durability cells carried over unchanged. **Its `critics-developer` pass found two more C1 breaks, both this stage's own.** The banking gate read `SnapshotCeiling`, which is capped by every core's pending-commit marker and that marker clears *after* the committer leaves the unresolved count - an interval in which a writer that committed above the walker's snapshot passes both loads - and a commit merely *pending* at the mint puts `snapshot_lsn` below the published maximum with no interval at all; it reads `CommitCeiling` now, raised inside `PublishCommit`'s hold and therefore strictly before the count drops. And `ApplyHeal` rebuilt from a handle whose count is fixed and then committed **wholesale**, destroying every entry the write hook appended while it walked - the tail the old edit-in-place heal kept for free and one store took away; `CabinStore::Rebuild` keeps it, in a fresh storage rather than a shrink, and takes the **handle** so a set replaced under the heal is left alone. The pass also deleted `SHOW CABINS`' `(held by core N)` gate, which one store makes a lie, a dead cap arm, an orphaned comment and a dead null test. **Four things recorded rather than fixed**, each in `known-gaps.md`: the cabin optimizer's build does not announce; a Cabin builds rarely while anything is unresolved anywhere, which is the honest price of an instance-wide gate; the CREATE decision still sees one core's scan shapes, the residue of *a peer-owned relation earns no `CABIN AUTO`*; and two cores recording one pattern can strand a trail in a colliding directory slot. `RecordAccess`' page-11 hold is a new cross-core serialisation point on the statement path, stated at the source and **not measured**. **Done-conditions**: a peer's Cabin observation is banked and served (cell above); the waystone and cabin contract suites stay byte-identical across configurations (64 contract cells, 304 cabin/waystone/stats cells green). The first - *`SHOW ACCESS` totals match the pre-change engine for the same workload* - is **not met as written**: no A/B run was executed, the overhead measurement being suspended. What was checked instead is that the arithmetic cannot differ - `RecordAccess` is the one writer either way and the batch's fold was a sum of the same executions - and that the per-shape counts the cells pin are unchanged. **Suite 3128/3128** (Debug). Overhead not measured |
| AT-S8 | **built 2026-09-23** on `at-s8-checkpoint-listener`: checkpoint and the listener. **The checkpoint's shape was the operator's**, because the row did not decide it and one reading was quiet-wrong: each core's `TransactionManager` holds its own live set, unlatched, and recovery's analysis needs every live transaction's undo head in some `CHECKPOINT_BEGIN` inside the replay range - so one instance checkpoint over one core's table would leave a peer's loser un-undone after a crash. Taken: **every core keeps its checkpointer and its own table, and the run is the instance's** - `wal::CheckpointGate` admits one checkpoint at a time, a cadence tick that finds another core's running skips, and each core's cadence starts `core_id/cores` of a period late, because unstaggered ticks meet every period and let one core win the gate for good, holding the fold (and so the anchor) at the others' completion checkpoints (the first review's L1). The declined alternative, one checkpointer over an instance table, needs a latch on every core's live set and `AppendUndo`. **`RemoteCheckpointAnchor` retires with ring kind 16**: a peer is handed `Expeditor`'s `SuperBlockCheckpointAnchor` and publishes into it itself, so page 0 becomes a two-writer page (the anchor's fold from any core, the trx-id carve on core 0) and a **superblock latch** covers each mutation and its encode, released before the sync - declared in `rules.md` §3 with its order, outer to page 0's page latch. **D25's freeze 10 → 9.** A peer's checkpointer is built at `Open` wherever it is handed an anchor, no longer at `AttachTransport`; the two cadence bodies became `Checkpointer::RunGated`. **Step 1b is the finding the stage is worth most for, and it predates it**: the first `critics-developer` pass found that `DevicePageStore::WriteBack` copied a frame under the structure latch alone and cleared its dirty mark and recLSN unconditionally, while a writer marks a frame dirty at the fetch, before it mutates - so since AM-S2 step 3 a checkpoint on one core could persist half of another core's write with a valid checksum and drop the mark of a write that had not gone out: a committed change the next checkpoint's dirty table no longer names and the redo start may pass, or (page 0) an unmountable volume. **Fixed on the operator's word, inside the stage**: each page is copied under its page latch, shared, one at a time and released before the WAL gate - a flush waits out a foreign exclusive holder (running, since no task parks holding a pin), the background drain tries once and leaves the frame dirty - and `Frame::dirty_gen`, drawn from the store's own counter at every dirty mark, makes the clean conditional on nothing re-dirtying the frame after the copy. The frame grows 32 → 40 bytes, a 16-bit generation being able to wrap inside one device write. The second review found the gap the first fix left: the fetch marks before it latches, so a copy could fall between the mark and the write, and page 0's unlogged writes never re-mark - an exclusive acquire now marks again (one structure-latch hold per exclusive fetch at `cores > 1`, **not measured**). **The listener**: `peer_listeners` becomes the arrangement (AT-R12, D19) - every peer listens, SO_REUSEPORT above one core and an exclusive bind at one - and **the TLS/SCRAM refusal is removed, not handed on** (AT-0 item 8): the context and the credential store were on `ServeRuntime` already, read-only after `Start` and outliving every peer, so one `TcpServer::ClientSetup` carries the channel, the gate, the identity source and the protocol to every listener. `CheckPeerListenerConfig` and the key retire; a retired key is refused by name ahead of the unknown-key check, which also made **AT-S6's successor-naming refusal of `in_doubt_ceiling_ms` reachable** - it sat below that check and had been refused as merely unknown, so AT-S6's third done-condition was false as recorded. **A peer's dispatcher ran under the dispatcher's defaults**: `indexes`, `max_insert_rows`, the aggregate limits, `sort_max_rows` and `join_build_max_rows` never reached it, so a session the kernel landed on a peer ran under limits the config file did not set; they travel as one `StatementLimits` now. **Cells**: the anchor published from a peer and decoded off page 0; a peer's tick skipping under a held gate; the gate under two threads; both page-0 writers waiting for the superblock latch; three writeback interleavings forced with a held device write or a held page; a peer listener asking the auth gate it is handed; a peer refusing past the sort cap it is handed; `Expeditor` handing a peer core 0's limits and a listener; both retired keys. **Mutants**: anchor latch, carve latch, peer gate, the copy's share, the clean's generation check - each killed, the concurrency ones 5/5; the re-mark's cell failed 5/5 against the unfixed source. **The optimizer surface, on the operator's word (option 1 of two)**: a peer session reached neither the physical optimizer nor the Cabin controller, and `SET CABIN_OPTIMIZER` on a peer answered OK and moved a flag nothing read. The pieces are the instance's now and every dispatcher is handed one `OptimizerSurface`: the relayout mode, the one collector, one `std::atomic<bool>` switch the cadence reads, and the controller and executor behind a **view latch** core 0's cadence holds across a tick and any core's `SHOW CABIN_OPTIMIZER` holds across its read. **Doing it found a data race from AT-S7**: the one Cabin store forwarded every core's lookups into `OptimizerSignals`, which had no synchronisation of its own - it is latched above one core now, innermost, the view latch outer to it (`rules.md` §3's new row). Cells: a peer's SET flipping the instance's switch and its SHOW seeing the controller; a note waiting for the collector's latch; `Expeditor` handing the peer one switch; mutants killed. **Recorded rather than fixed**, each in `known-gaps.md`: a multi-core test instance's probed port is now bound with SO_REUSEPORT; no cell drives a stale writeback against a re-faulted frame. **Declined**: building the debug text listener from `ClientSetup` (a behaviour change to that port's durability class and identity source, outside the stage); dropping `CheckpointGate::skipped()`, which the gate's cells read. **Done-conditions**: *two cores cannot run a checkpoint at once* - held, by the gate and its cells; *a session accepted on any core runs to completion there* - held, under core 0's limits and core 0's optimizer surface; *TLS with per-core listeners* - held, the refusal gone. Suite 3135/3135 (Debug) at `8a9daec`, and the tree at the rows' commit below. Overhead not measured |
| AT-S9 | **built 2026-09-24** on `at-s9-placement-retired`: placement and `owner_core` (D17, D18, E8's verb, E9's withdrawal). **Four rulings were the operator's**, the row having left three open and one it could not keep: (1) **the bytes are reserved, not dropped** - `sys.tables.owner_core` and `sys.ranges.owner_core` are written 0 and never read and no row size changes, so AT-R7's four anchors do not move and AT has no format event here; `SysTableRow` gains the literal offset asserts it lacked (80, 84, 85, 93, 97, 101, 102; size 106); (2) **AT-0 item 4 is answered *struck***: both remote routes retire - the single-step fan-in over a split relation and the two-step join pipeline - and the `kStep*` services and the transport stay, with no engine producer, for AT-S10/S11 to delete; (3) **insert spreading retires**: `range_size_ids` is refused by name, no range is opened, and a relation split before AT-S9 is read and written whole on whichever core the session is on; (4) **the core count is unpinned** (the question E9's withdrawal handed here): nothing names a core on disk any more, so a mount at another `cores` records the running count (`SuperBlock::SetCoreCount`, logged at Info) where it used to be refused - sound because one stream publishes anchor slot 0 alone. CLA's own proposal, stated to the operator: **D18's affinity statistic is not stored**, because nothing reads it until AS-E - NS10's verb becomes *"declares the affinity of"* and the declaration has no consumer. **Retired**: `core_placement.hpp`, `PlacementPolicy` and the `placement` key (refused by name), namespace placement, the publish hook, F5's colocation check, `range_alloc` (range opening, the IS1 pump, IS3's routing, `CheckRangePlacement`, `WriteTargetCore`), `range_eligible`, `core_affinity` (`CheckReadAffinity`, the fan-in, the pipeline route, the dispatcher's remote-read completion), the cross-core write counters and their `SHOW META` block, DESCRIBE's `owner_core=`, the session's home core (CC3), the catalog's core count and core id, and the row-id grant's range opening; `TableAccess::WalkHeads(PkSpan)` replaces `WalkHeadsFor`, `AllWalkHeads`, `ServableBy` and `RangeOwnerFor`, and a Cabin serves a split relation. **A live defect, closed by the retirement rather than fixed**: the two-step pipeline ran a join's stages under an autocommit snapshot, so a join inside `BEGIN` on a peer could not see the transaction's own writes - the shape AT-S6 fixed for a single-relation read, left open for a join. Pinned by `ShippedReadOwnWrite.AJoinInsideATransactionOnAPeerSeesItsOwnWrites`, which **was not run against the pre-change engine**: the review traced by reading that it would have failed there. **A pre-existing gap recorded, not fixed** (`known-gaps.md`, AT-S4's): a peer's leased id block can be refused `OutOfRange` by an unsplit heap chain, since AT-S5; on a pre-AT split relation every leased block now lands in the top range, so it applies there too. **Its first `critics-developer` pass** found no engine defect: one broken tool (`multicore_benchmark.py` read `owner_core=` off DESCRIBE and failed at setup; under `--peer-listeners` writers are spread round-robin now), about 900 lines of dead code (deleted), and stale prose (corrected); no finding rejected; `OpenRangeRows` and `CreateRangeEntryPage` are kept, with no production caller, for the cells that build a pre-AT split relation. **Its second pass**, over `cffbaa7`, found no engine defect and nothing reachable deleted: the tool reported every writer on core 0 though every core accepts since AT-S8 (it reads each writer's core now), and its `--placement` flag was accepted and did nothing, so the placement layer went; seven comments and five includes the deletion left behind were corrected or removed; no finding rejected. **Cells**: 69 retired with the premise that went - 37 with the two deleted suites (`range_alloc` 23, `range_eligible` 14) and 32 in adapted files - and 7 more with `core_affinity_test.cpp` at the review; 12 renamed to the claim that now holds; 1 new. **Done-conditions**: *a pre-AT volume mounts and its `owner_core` values are ignored - no refusal* - held, `TheRetiredOwnerCoreWordIsWrittenZeroAndNeverRead`; *`ns.table` still resolves; namespaces still exist as names* - held, the namespace cells unchanged. Suite 3063/3063 (Debug; one pre-existing disabled cell) at `cffbaa7` and again at `fa58221`. Overhead not measured |
| AT-S10d | **built 2026-09-25** on `at-s10d-transport-retired` (`6c643d4`, prose `fc8a867`, review `6a45d21`): **the ring transport retires - AU-S6's whole list, carried as S10's last sub-stage** as `59ed9c0` named it, so **AT-S11 is struck** on the operator's word (2026-09-25). Deleted: `ring_message.hpp` (header, kind enum, D25's census at 2 - kinds 18 and 22 went with it), `ring_transport.{hpp,cpp}` (the seam and the real N² transport with its preallocation), `sim_ring_transport.{hpp,cpp}`, `spsc_ring.{hpp,cpp}`, `send_retry.hpp` and the four transport test files; `Scheduler::AttachTransport`, `RegisterMessageHandler`, phase 3's `DrainInbox`, `max_messages_per_iteration`, `messages_drained()`; `CoreRuntime::AttachTransport`; `Expeditor`'s transport and `transport()`. **The wake loses its re-check**: the pre-block look was the ring's `HasPending`, and with it the `seq_cst` fence after the flag's store and its twin in `WakerTable::Kick`; the flag is a relaxed hint. **A choice, stated as one** (the review corrected the first draft, which called it forced): D19's `Pending(core)` is a predicate the reactor could re-poll after raising the flag, and not doing so costs a kick that races the destination into its block one idle block (`max_idle_block_ms`); every consumer is level-triggered. **`SHOW META` loses `sched_wake_race_skips` and `sched_spurious_wakes`**, and `client-manual.md` says so. Specs: `sched.md` §2 (phase 3 kept as a number, retired), §5 rewritten as write-then-kick, §7, the park census (the shipped statement's row struck, the lock family's two parks recorded as owed to it), §8, invariant 7; `crosscore.md` §3, §4, CC6, §8; `rules.md` §3's wake row; **G1's sentence** takes AR0-6's wording; `CLAUDE.md`'s cross-core row. **Its `critics-developer` pass** found the fence removal sound and **one defect the stage introduced**: `AttachTransport` was one of the two places a reactor learned its core id, and `RunOnce` sets `CurrentCore()` from it, so a runtime attached to no registry - every peer in `core_runtime_test.cpp` - ran as core 0 (re-entering core 0's exclusive page latches, stamping core 0's stream); production attaches every peer. Fixed in `CoreRuntime::Open` (`AttachWakerTable(nullptr, id)`) and pinned; a tautological D7 assertion fixed; stale prose in eight headers; nothing rejected. Cells: 41 retired (29 transport, 7 `SchedulerInboxTest`, 4 send-driven wake cells, 1 coroutine request/response), 2 renamed, 4 re-pointed. The golden log is unchanged (`WalGoldenLog.TheSingleCoreScriptWritesThePinnedBytes`). **Suite 2956/2956** (Debug, one pre-existing disabled cell), from 2997 at `ba8c824` less the 41. Overhead not measured |
| AT-S12 | **built 2026-09-25** on `at-s12-prose-sweep` (code comments `f48d213`, prose `007b5b5`, review `a96d53f`): the prose sweep, to the rule and not to §3's command. **Code**: comments in 56 files, no behaviour change - the four `(M5)`/bare-M5 sites, `core_runtime.hpp`'s asymmetries 1-3 struck and its "core 0 owns the superblock, the free map, the catalog pages and the listener" rewritten, the catalog swept for owner-shaped justifications (AT-7 item 16), and fifteen cells renamed to the claim they pin (one, `APeerReadsTheCatalogAndCannotWriteIt`, asserted the opposite). **Specs**: `crosscore.md` (preamble; CC1, CC5, CC11; §2a, §6, §6a, §6b collapsed to the rule with `2b20369` as the pointer; §8) - CC11 and CC13 were already current; `physical-optimizer.md` (one controller for the instance; R6's mover exclusion recorded undecided); `page.md` §6; `rules.md` §3 gains the Cabin store's row; `namespace.md` and `sched.md` §5 already current; beside them `eviction.md`, `aggregate.md`, `bulkinsert.md`, `client-manual.md`, `cabin.md`, `foreign-keys.md`, `ddl-transactional.md`, `txn.md`, `parser-v2.md`, `keystoneid-invariant.md`, the manual, `CLAUDE.md`. **Done-conditions, checked as greps** at `a96d53f`: `grep -rn -E '\(M5\)\|\bM5\b'` over `include src tests sim tools docs/spec docs/rules manual CLAUDE.md`, less `FK-M5`/`K-M5`/`AR0-M5`, answers nothing; every hit of `ring_message\|RingTransport\|kTrxIdLease\|kRowIdLease\|MessageHeader\|AttachTransport` is a record of what went; every hit of `owning core\|owner core\|on (its\|the) owner\|owned by core\|core 0 alone\|peer-owned\|home core` is marked past or is the page latch's or a visibility slot's owner. **Found by reading, not reproduced - three quiet wrong answers AT-S5 made reachable and nothing closed**: a secondary index's insert descent is not re-validated (`docs/inflight/bugs/a-secondary-index-descent-is-not-revalidated-across-cores.md`); **the clustered tree's divide walks up an unchecked recorded path** - AT-S5c closed its leaf window only, and the clustered tree is the storage default (`a-btree-divide-can-promote-into-a-parent-another-core-divided.md`, the review's finding); two cores can create one name twice (`two-cores-can-create-one-name-twice.md`). `known-gaps.md`: the Cabin store's latches at cores = 1 added (G1/G2), "the CREATE decision still sees one core" struck (AT-S8 step 3 closed it). **Dead code listed for the operator, not removed**: `MayWrite` (always true), `CommitAck`, `AdoptSnapshot`, the prepared-transaction state, `LogTxnPrepare`, `AnyCabin()`, among others (`f48d213`'s message). Suite 2956/2956 (Debug, one pre-existing disabled cell) at each of the three commits. Overhead not measured |
| AT-S4, AT-S10, AT-S13 (AT-S11 struck) | not started; each gated on the operator's word (S4 after S5, AT-7 item 12) |
