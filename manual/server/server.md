...
| `in_doubt_ceiling_ms` | `200` | **Inert since AO-S3, and kept only so a configuration carrying it still mounts.** It used to bound how long a write waited for a row held by a transaction this core had prepared for a cross-owner commit, and `0` meant "refuse at once". A write now waits for **any** holder that has not decided, and the wait ends when the holder decides — not on a clock — so nothing reads this key. What bounds a wait that goes wrong is the lock-wait fault net, an 11 s `constexpr` that fires only on a fault and is logged as one. M2's M3 stage re-scopes and renames this key rather than leaving a second name for a quantity (`workorder-ao-m2-lock-family.md` AO-R8). |
