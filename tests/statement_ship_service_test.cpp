...
TEST_F(StatementShipTest, TheLongestShippableStatementIsTheSlotMinusTheHeader) {
    ...
    // AN-S3's snapshot eight more, and `client-manual.md` says 968 because
    // of this.
...
TEST_F(StatementShipTest, TheJoinBitCrossesOnTheRequestLeg) {
    // **RR0's field, on the request leg.** `join` goes out with the request
    // - it is the coordinator's statement about a context it believes
    // exists. The reply leg used to carry the participant's watermark back
    // (the order's HR2); AN-S2 removed it with the field it was read from,
    // and the reply now carries the status and the text alone.
    auto plain = ShippedStatementRequestOf(99, 1, 4000, Role::kReadWrite, "SELECT 1");
    ASSERT_TRUE(plain.ok()) << plain.status().message();
