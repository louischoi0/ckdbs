...
`docs/spec-types.md`, `docs/feat-index.md`, `docs/feat-aggregate.md`,
`docs/parser-v2-workplan.md` V08.
...
**It contradicts invariant 13.** `docs/rule-fixed-length-tuple.md` §2: a
...
from NULL in `VARCHAR2`. `docs/rule-fixed-length-tuple.md` §3 gives exactly
...
in the Bound Cabin entry's zero padding (`docs/feat-assertion.md` §5.1).
...
- **`IN` / `NOT IN` (V08, `docs/parser-v2-workplan.md`).** `NOT IN` over a list
...
- **Secondary indexes** (`docs/feat-index.md` §13). Whether a NULL key is
  ...
  oversight. `[OPEN]`.
- **Aggregates** (`docs/feat-aggregate.md`). `COUNT(*)` counts rows,
...
- **Assertions** (`docs/feat-assertion.md`). A Bound Cabin group key derives
...
- **Foreign keys** (`docs/impl-foreign-keys.md`). Already anticipated:
  `kFkNullable` exists at `include/kds/catalog/rows.hpp:841`. A NULL child key
  satisfies the constraint vacuously.
- **The wire and the client** (`docs/client-manual.md`,
  `docs/protocol-wp.md`). The text protocol needs a rendering for NULL
