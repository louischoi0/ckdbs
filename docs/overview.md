...
- **Super column:** every tuple's mandatory first column is one 64-bit word: `id:40 | flags:8 | meta_handle:16`. Flags byte is the transaction byte (Oracle lock-byte style); tuple liveness status lives in the slot directory instead.
...
4. Super column encoding is exactly `id:40 | flags:8 | meta_handle:16`; the word is read/written as an atomic `uint64_t` (CAS for updates); fields never tear.
