...
| M5 | Shared-resource ownership | **Core 0 is the system core**: owns the superblock (page 0), free map, file growth, extent leasing, and catalog pages. Other cores hold extent leases and catalog caches; checkpoint anchors are written via message to core 0 (`SuperBlock::SetWalAnchor(core_id, …)` already takes the id). **Ratified as the access rule 2026-08-30** (`crosscore.md` CC11, blueprint §8 closed): what M5 recorded as an ownership arrangement is now the decided rule — **every core reads the fixed system range with the same authority, core 0 alone writes it** — with both of blueprint §8's candidates, a partition-boundary lock and a rotating coordinator, declined. M5 is therefore not a residue to retire for those three structures; it is the answer. File growth and extent leasing stay core 0's by the same decision |
