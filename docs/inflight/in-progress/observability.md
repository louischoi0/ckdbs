...
### 8a. Three times now, a missing instrument has blocked an attribution

Recorded here rather than in any of the measurements, because the subsystem
this document proposes is what owns the gap, and because a second instance
is what turns an anecdote into a requirement — the third arrived within the
same day.
...
- **RR2** (`bench/v2.5.0/results-rr-read-half-*.md` §7), the same day,
  sweeping D5's in-doubt ceiling: a blocked writer's p0 tracks the
  configured ceiling within a few percent, and its p50/p99 run **2-6x
  higher**. The likely cause is reactor queueing on the shared core, and it
  is recorded as *likely* because nothing can tell a queued wake from a
  slow leg from outside the process.

All three are per-*request* attribution inside one statement, which §8 above
is exactly the argument for: a counter that says "how many syncs since boot"
