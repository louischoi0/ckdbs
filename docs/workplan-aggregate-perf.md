...
Tasks `AP01`-`AP06`, companion to `docs/feat-aggregate.md`. Every item here
...
first reader, which `docs/heap-and-tuple.md` §7 has wanted since the
statistics landed.
...
- **Parallel or partial aggregation.** `Aggregator::Merge` (AG-M) is built
  ...
  `docs/crosscore.md`'s pipeline rather than here — there is no second core
  running steps yet.
