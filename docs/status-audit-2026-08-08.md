...
1. **`docs/parser-v2.md:9` — "Status of the work: unbuilt."** The most
...
2. **`docs/keystoneid-invariant.md:7` — "K-M2..K-M6 not started."**
...
3. **`docs/wal.md:5` — "logging works for INSERT."** Understates: the
   transaction work logs `UPDATE` and `DELETE` as well
   (`UNDO_WRITE` → `HEAP_OVERWRITE`, and `HEAP_DELETE_MARK`), which
   `docs/txn.md` and `include/kds/txn/undo_log.hpp` carry. The second half of
...
| IX15 documentation sweep | AGREES | `docs/feat-index.md` header, `CLAUDE.md` Core Architecture entry |
...
### `docs/waystone-concpets.md` / `docs/waystone-workplan.md`
...
### `docs/cabin-workplan.md` / `docs/feat-cabin.md`
...
### `docs/txn-workplan.md` / `docs/txn.md`
...
| §8's gap: MVCC ships before recovery | AGREES | no recovery entry point exists; `docs/wal.md` §12 says the same |
...
### `docs/workplan-aggregate.md` / `docs/feat-aggregate.md`
...
### `docs/workplan-crosscore.md` / `docs/crosscore.md` / `docs/sched.md`
...
### `docs/protocol.md` / `docs/protocol-wp.md`
...
| P01 (client manual rewritten for KWP/1) implicitly outstanding | AGREES | `docs/client-manual.md` still documents the newline protocol, as `protocol.md`'s own status says it should |
...
### `docs/keystoneid-invariant.md` / `docs/keystoneid-k0-findings.md`

| claim | class | evidence |
|---|---|---|
| "K-M1 done 2026-08-03" | AGREES | `tests/keystone_id_test.cpp`, `docs/keystoneid-k0-findings.md` |
...
| "K1 does not hold across a crash today" | AGREES | `sys.tables.next_id` is unlogged and no recovery exists; consistent with `docs/wal.md` |
| findings' measurement of allocator cost | UNVERIFIABLE | `bench/results-keystone-alloc.md` exists; the numbers are a measurement |

### `docs/impl-foreign-keys.md`
...
### `docs/heap-and-tuple.md` — authoritative spec
...
1. **`docs/parser-v2.md:9` is the one line worth changing today.** It is the
