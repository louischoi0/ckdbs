...
Normative coding rules for the KDS storage engine. Rules only: design rationale and architecture live in `docs/heap-and-tuple.md`, and the agent working guide is `CLAUDE.md`. Every rule here is binding; the final section lists rule areas that are **not yet decided** — do not invent policy for them.
...
- **Compiler bitfields are forbidden** in on-disk formats. Packed fields (e.g., the Keystone column `id:40 | flags:8 | reserved:16`; see `docs/heap-and-tuple.md` §4) are encoded/decoded with explicit shift/mask `constexpr` helpers.
