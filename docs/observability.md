...
- **Inspection commands**, in the same `\n`-escaped one-line convention as `SHOW PAGE` (`docs/client-manual.md`):
...
- Whether `SHOW TRACE*` survives the move to KWP/1 (`docs/protocol.md`) as protocol messages, or stays a text-protocol debug affordance and disappears with it.
