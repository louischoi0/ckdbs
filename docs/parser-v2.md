...
**I11 — `ORDER BY <pk> [ASC|DESC] LIMIT n [OFFSET m]`.** Non-pk ordering is `Unsupported` — the semi-sorted heap and the B+ tree make pk order the free order. Limit and offset are slots, so `LIMIT 10` and `LIMIT 20` share a `pattern_id`. `LIMIT` is not implementable until the walk contract in I15 changes.
