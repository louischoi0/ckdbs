...
A join contributes one `Lookup`/`Probe`/`Scan` step per relation in written order. This table *is* `docs/waystone-concpets.md` §2's trust model — a waystone may replace a lookup, never a search — extended with the negation rule: **negation steps are search-class by definition**.
...
**I11 — `ORDER BY <pk> [ASC|DESC] LIMIT n [OFFSET m]`. `[AMENDED 2026-08-10 — parse half built, V09]`** Non-pk ordering is `Unsupported` — the semi-sorted heap and the B+ tree make pk order the free order. Limit and offset are slots, so `LIMIT 10` and `LIMIT 20` share a `pattern_id` — corpus-pinned now. Three decisions rode the parse half. **Each clause is independently optional**: `LIMIT` without `ORDER BY` is well-defined here in a way general SQL cannot promise, because I12 already makes emission order a client contract, so the clause takes a prefix of an order the statement has. **`DESC` parses in no production and answers `Unsupported`** — every chain links forward only; the word stays in this grammar so the refusal does not shift when a reverse walk lands. **The tail is the outermost non-aggregated block's**: over aggregated output it is `Unsupported` (fold order is not a contract; belongs with HAVING in `feat-aggregate.md` §10's post-fold-consumer decision), and inside a subquery it is `Unsupported` with the byte. Pk-ness of the ORDER BY column is the compile half's check, not the parser's. The stale closing claim is retired: I15 R4 (stoppable walks, V03) landed long ago, and `LIMIT`'s execution is a sink-decorator quota over `RowSink`'s `kStop` — `exec::EmissionQuota`, built with the compile half the same day. The defining contract: the reply to `LIMIT n OFFSET m` is rows [m, m+n) of the unlimited reply.
...
**Decorrelation rewrites are forbidden**, not merely unimplemented: turning a correlated subquery into a semi-join or hashing its inner side once is exactly the silent reordering this contract rules out, and it is not validatable against the trail model. The one permitted shortcut, because it changes no result and no order: a false uncorrelated top-level `EXISTS` short-circuits the statement without opening the outer relation.
...
- **Replay** consults the instance's trail for entries with the step's `step_id` and applies `docs/waystone-concpets.md` §2 per entry; any miss falls through to the authoritative path *for that step alone*. Search-class steps use the trail only as a prefetch batch.
...
The check is free at runtime: the producing row is already decoded by R1 and the probe key is already a resolved `ColumnRef`. It is mandatory before any `Probe` replay ships, it lives in **one** function shared by every replay path, and it requires a `docs/waystone-concpets.md` §2 amendment plus a §11 test — update the producing row's key column between record and replay and require the *new* row.
