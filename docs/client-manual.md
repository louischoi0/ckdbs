...
| `SHOW META` | none | `version=<n> create_time=<n> last_mount_time=<n> wal_anchor_count=<n> cabin_optimizer=<on\|off>` plus the recovery block below | Dumps the superblock. Times are Unix seconds; `wal_anchor_count` is how many per-core WAL anchors the database carries (`docs/wal.md` §14-3). |
