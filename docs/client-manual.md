...
| `SELECT * FROM <name> [WHERE <cond> [AND <cond>]*]` | table name, optional AND-only WHERE | header line + one row per match | With Waystone enabled and a WHERE that is exactly one equality against the pk column, this is an O(1) probe - measured at 8,417 qps / 11 µs server-side against 311 qps / 2,582 µs for the scan at 5,000 rows, and **flat** in row count (12 µs at both 2,000 and 20,000 rows). Anything else - no Waystone, an extra `AND`, a non-pk column, a range - is a full scan of the table's whole page chain, in chain order - which is primary-key order page by page, so rows come back roughly pk-sorted without anything sorting them. Still a scan: no index, and no `min_key` pruning of pages the `WHERE` cannot match. See below for the `\n`-escaping convention this reuses from `SHOW PAGE`. No projection - column list is always `*`. |
| `WAYSTONE ENABLE \| DISABLE \| STATUS [<name>]` | verb, table name (optional for `STATUS`) | `OK ...` / counters line / `ERR ...` | Turns the per-relation Waystone on or off (`docs/waystone-concpets.md` §7). While on, `INSERT` additionally writes one 32-byte entry (O(1), no fsync) and `SELECT ... WHERE id = <n>` probes for the row's location instead of scanning the chain - a validated hint, so a miss, a stale entry, or a wrong Keystone id at the target falls through to the same scan and **the result is identical either way**. `ENABLE` is refused on a relation that already holds rows: backfill is not implemented, and partial coverage a reader might trust is worse than none. `DISABLE` orphans the directory and entry pages rather than freeing them (no free-page path yet) and re-enabling allocates a fresh directory. `STATUS` reports server-wide `probe_hits`/`probe_misses`, plus this relation's state, root and depth when given a name. A dispatcher command rather than `CREATE TABLE` syntax because the parser is being replaced (`docs/parser.md`); KWP/1 decides the real spelling. |
