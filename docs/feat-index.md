...
tuple (`docs/txn.md`): `OverwriteTuple` keeps `(page_id, slot)`, and undo
...
- **Nothing reclaims.** A superseded entry costs memory, a page, and a
  ...
  deliberately unregistered (`docs/txn.md` §9). Same bargain the var-heap and
...
other statement failure does (`docs/txn.md`: failure atomicity is per
...
DDL is not transactional (`docs/txn.md` §7): index catalog rows are stamped
