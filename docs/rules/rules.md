...
- **Shared-nothing is no longer the rule it was.** "Shared mutable state across cores is forbidden" was the rule until AR0 M0; it is now "shared mutable state across cores is *declared*". A structure that is shared and undeclared is a defect, and so is a lock in a subsystem whose spec does not carry one — the change widened what may be shared, not who may decide it. **The declaration lives in the owning spec, and this list is an index of them rather than the authority**:
  ...
  | The **free map** — the region bitmaps, one set for the instance since the pool became one (AM-S2 step 3, AW-S1b) | the store's frame-table structure latch on the allocation path (`ClaimNextFreeIdLocked` scans and marks under one hold), and **nothing on three others**: `IsAllocated` reads a region's bytes unlatched, `EnsureRegionResident` creates a region outside the hold because it grows the device, and `FlushMaps` walks `map_regions_` unlatched while another core may be inserting into it. **Declared as it is, not as it should be** — the third is a filed defect (`docs/inflight/bugs/flushmaps-lease-guard-and-unlatched-region-walk.md`, AM-S3's), and until AM-S3 answers "who writes the map back when one store serves every core" this row is a statement about an unfinished structure rather than a protocol | `docs/spec/page.md` §5, `docs/spec/crosscore.md` CC11 |
