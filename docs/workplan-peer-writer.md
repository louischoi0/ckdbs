...
| PW1c-4 | Exact-page write grants in `DevicePageStore`; CC7's publish upgraded to send them after the handoff record is durable; **the receive side performs §9 rule 6's durable restamp before admitting any write** — the grant is not applied until the page carries the receiver's stamp and a `page_lsn` in its space. Carries three small debts PW1c-2's review deferred to it by name: the rule-1a flush clears the pool's dirty entry (the checkpoint-reseed precondition above); `skipped_not_dirty` lifted into `MountRecovery` beside `redo_skipped_by_lsn`, or its handoff-free-zero claim stays untestable outside gtest; and the two unreachable-today analysis cases get their one-liners (two handoffs for one page; a hand-built handoff with a nonzero txn_id, which today mints a phantom loser) | PW1c-1..3 |
