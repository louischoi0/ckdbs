...
| PW1c-2 | **Built 2026-08-24** (same tree): analysis *removes* a handed-off page from the outgoing stream's dirty page table — a checkpoint-seeded entry included, and a page that returns (A→B→A) re-enters at its post-return recLSN by erase-then-first-wins. **The redo half was the finding**: `Redo` consumed only `redo_start_lsn` and never consulted the dirty page table, so rule 3's "redo never touches it" was unenforced — a departed page's records would have replayed through an RV5 gate comparing incomparable LSN spaces, the PL spec's §3 failure. Redo now applies a page record only when the page is in the table and the record's LSN is at or above its recLSN (the ARIES filter), decided **before** the page load so the page is never faulted, counted by `skipped_not_dirty` — zero on any stream with no handoff, since an ordinary scanned record never sits below its own page's recLSN. Pinned by four tests: removal, the return recLSN, the seeded entry, and redo skipping a departed page unfaulted with the store never holding it. **Two residuals its review named, neither closed here**: the returning page's (A→B→A) post-return records still pass an RV5 gate that may compare against the *other* stream's `page_lsn` stamp — PW1c-3 is what makes them comparable, and §9 rule 5 as phrased does not cover the returned page (analysis *did* see a handoff), so PW1c-3 must handle it explicitly; and the erase is positional, so a later `CHECKPOINT_BEGIN` still listing the page re-seeds it — PW1c-4's rule-1a flush must clear the pool's dirty entry, not merely write the bytes | PW1c-1 |
| PW1c-3 | **Built 2026-08-24** (worktree `pw1c1-handoff-record`): `page_flags` carries `core_id + 1`, stamped wherever `page_lsn` is — `DevicePageStore::StampPageLsn`, the funnel every logged mutation rides, and redo's apply; 0 stays "never stamped", no backfill, and an unstamped page takes today's RV5 comparison unchanged. Redo's reachable-mismatch check: a foreign stamp with no handoff seen is `Corruption` at mount (rule 5); a foreign stamp on a page analysis *did* see leave is the **returned page** — its admitted records apply over the incomparable `page_lsn` and the first apply restamps the own stream, PL §9's new rule 5a, added with this task, which closes the residual PW1c-2's review named. `page_mgr`'s Frame is not production-wired and carries a named debt comment instead of a stamp | PW1c-1 |
| PW1c-4 | Exact-page write grants in `DevicePageStore`; CC7's publish upgraded to send them after the handoff record is durable. Carries three small debts PW1c-2's review deferred to it by name: the rule-1a flush clears the pool's dirty entry (the checkpoint-reseed precondition above); `skipped_not_dirty` lifted into `MountRecovery` beside `redo_skipped_by_lsn`, or its handoff-free-zero claim stays untestable outside gtest; and the two unreachable-today analysis cases get their one-liners (two handoffs for one page; a hand-built handoff with a nonzero txn_id, which today mints a phantom loser) | PW1c-1..3 |
