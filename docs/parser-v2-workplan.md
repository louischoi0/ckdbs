...
**Vehicle (J4).** Phases V-2 through V-5 extend the **current** recursive-descent parser and its owning AST. The blueprint parser — arena, flat AST, zero-copy tokens, slot table, binding at PARSE — lands afterwards, in phase V-6, and its acceptance criterion is that it emits **identical `StepChain`s** over V23's corpus. The `Compile(AST) → StepChain` contract (V14) is the seam that makes that a checkable statement rather than a hope.
