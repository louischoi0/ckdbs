...
| `SHOW META`'s group-commit batch (2026-09-02) | none | `wal_group_commits=<n> wal_group_batches=<n> wal_mean_group_batch=<f.fff>` | **How many commits this core's fsyncs are amortised over.** D2's whole mechanism is that one sync resolves a batch, and `kds.conf.sample` states the edge that makes it matter: *"a batch of one is a batch"*. `wal_group_commits` is D2 commits registered on this core, `wal_group_batches` the syncs that resolved at least one, and the quotient is the mean batch. **`1.000` is the cliff** - every commit paid its own device sync, which is `strict` wearing `group`'s name; AF-T5 measured that as **2.35x on a load phase** against a layout that gave each core two committing sessions (`bench/v2.8.0/results-af-t5-namespace-grouping-*` §3b). `0.000` means **no batch has completed**, which is a different fact from a batch of one and prints differently: it is what `relaxed` shows, since D3 registers no group commit at all. Core-local and cumulative from mount, like the sync counters above, so a reading is a before/after delta. **What decides the batch is sessions committing on this core**, and a shipped write commits on its relation's *owner* - so on a single-listener instance the cores paying un-batched syncs are the peers, and reading them needs a session there (`peer_listeners = on`). A count of attached sessions would answer the opposite question and is deliberately not the field. **Absent** wherever the sync counters are. |
