...
| `SHOW META`'s group-commit batch (2026-09-02) | none | `wal_group_commits=<n> wal_group_batches=<n> wal_mean_group_batch=<f.fff>` | **How many commits this core's fsyncs are amortised over.** D2's whole mechanism is that one sync resolves a batch, and `kds.conf.sample` states the edge that makes it matter: *"a batch of one is a batch"*. `wal_group_commits` is D2 commits registered on this core, `wal_group_batches` the syncs that resolved at least one, and the quotient is the mean batch. **`1.000` is the cliff** - every commit paid its own device sync, which is `strict` wearing `group`'s name. **Measured behaviour** (`bench/peer_group_batch_probe.py`, §3c of `bench/v2.8.0/results-af-t5-namespace-grouping-*`): it reads **1.000 at one *and at two* concurrent committers on a core**, 2.000 at four and ~4.000 at eight - the batch is `n/2` from four upward and flat below, on a peer and on core 0 alike. So a core with one or two committing sessions is paying `strict`'s device cost under `group`'s name, and the field is how that is seen before a workload is designed around it. **`wal_group_batch_max` / `wal_group_batch_min` are the spread beside the mean** (added the same day, for one question the mean cannot answer: was the amortisation the same every time?). `min` is over non-empty batches, so `min == max` is a batch that never varied and `0` means none has completed. First reading, 8 concurrent committers on one core: mean 3.974, **min 1, max 5** - it clusters near `n/2`, never reaches `n`, and dips at the run's edges. `0.000` means **no batch has completed**, which is a different fact from a batch of one and prints differently: it is what `relaxed` shows, since D3 registers no group commit at all. Core-local and cumulative from mount, like the sync counters above, so a reading is a before/after delta. A shipped write commits on its relation's *owner*, so reading the core that actually paid needs a session there (`peer_listeners = on`) - **verified rather than assumed**: a session accepted on core `c` gets core `c`'s counters back. A count of attached sessions would answer the opposite question and is deliberately not the field. **Absent** wherever the sync counters are. |
| `SHOW META`'s WAL sync counters (2026-08-31) | none | `wal_syncs=<n> wal_interval_syncs=<n> wal_sync_failures=<n>` | What this core's log cost the **device** (XD0, `instructions/v2.7.1/measurement-xd.md`). Core-local, like every counter here, and **cumulative from mount** - including recovery's own syncs - so a reading is a before/after delta, never a total. `wal_syncs` is every device sync this core performed, from both places that perform one: `WalManager::Sync()` on the reactor, which is every sync somebody is **parked** on (a commit under `strict` or `group`, a cross-owner prepare, a client `SYNC`, the checkpoint gate), and the WAL writer thread, which takes D3's loss-window tick alone. **`wal_syncs - wal_interval_syncs` is the part somebody was parked on**, and that rule holds on every core: `wal_interval_syncs` counts the loss-window tick whether it was handed to the writer thread (core 0) or taken inline because there is no writer thread (**every peer core**, which starts none). On core 0 the two differ by the sync in flight and by any pair the writer coalesced, since the tick is counted where it is *requested* and the writer syncs where it *completes*. A sync is a **durability point, not a syscall**: one increment issues one `fdatasync` per open segment, so an N-segment log costs N syscalls per count, and the two fsyncs a *new segment* costs are in neither counter. **A cross-owner transaction under `group` performs three device syncs and *waits* for two of them** since 2026-08-31 (`docs/spec/cross-owner-txn.md` §2): the participant acknowledges the decide at its COMMIT append and the next drain carries that record, so `wal_syncs` is unmoved by that change and was never the field that would show it — it counts syncs performed, never syncs waited on. `wal_sync_failures` should be 0; it also counts a failed **flush**, so a non-zero value is not proof a sync was attempted. **Absent** where the dispatcher has no WAL at all - an embedded caller, never a served core. |
...
| `SHOW PATTERNS` | none | `patterns=<n>`, then one `\n`-escaped section per `sys.patterns` row | Each section carries `pattern_id=0x<hex> oid=<n> origin=<user\|auto> pinned=<yes\|no> class=<n> uses=<n> last_seen=<n> waystone=<root=<n>,depth=<n>\|none>`. Every pattern is identified by its hex `pattern_id`, and `origin` and `pinned` read `auto` and `no` on every row: their only writer was `CREATE PATTERN`, withdrawn 2026-08-31, and the two fields stay on disk. A `name=<s> params=<n>` pair was joined in for **declared** patterns and went with them; an auto-registered pattern never had a name to print. Rows left behind by an older fingerprint revision are listed too, marked `stale=v<n>` — they are the dead weight a version bump leaves for retention, and seeing them is the point. Inspection only. |
...
| `DROP PATTERN <name>` | — | `ERR only DROP TABLE, DROP CABIN, DROP INDEX and DROP ASSERTION are supported` | **Withdrawn 2026-08-31**, with `CREATE PATTERN` above. |
...
generic refusal. The list shrank on 2026-08-31, when `DROP PATTERN` was
withdrawn with declared patterns.
