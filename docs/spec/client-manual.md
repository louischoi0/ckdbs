...
- **A connection that drops mid-statement may find the statement applied.**
  ...
  whatever happens to the connection. The same holds for the ten-second
...
- **`REPEATABLE READ` does not cross cores as a single instant**
  ...
  *inside* a transaction is answered on an **answer edge** that streams its
  rows in batches, so the number of rows is not bounded; what is bounded is
  the **widest single row** — about 1,000 bytes, reachable at defaults with
  roughly fifteen full-width `varchar` columns — because a batch is one
  ...
  **longest statement that may be carried to another core is 976 bytes**;
  ...
  nothing*). And `ANALYZE` of another core's relation is refused, because
  the answer would describe a run this core did not perform.
  ...
  **`READ COMMITTED` makes no cross-core promise at all**, and it is the
  ...
  already lets every statement see the latest committed state, so a
  `READ COMMITTED` transaction reading two cores is reading each of them as
  it is now.
...
| `SYNC` | none | `OK synced` | Syncs the WAL, then writes the page store back to the data file. A logged statement is already durable to the configured `durability` class at commit; `SYNC` forces the device sync and the page write-back now. |
...
| `SHOW META` | none | `version=<n> create_time=<n> last_mount_time=<n> wal_topology=<single\|per-core> [wal_anchor_count=<n>] cabin_optimizer=<on\|off> core=<n>` plus the blocks below | Dumps the superblock. Times are Unix seconds. `wal_topology` is what this volume's log **is** — `single`, one stream for the instance, which every database created by this build or later has; or `per-core`, one per core, which a volume written before that keeps for life. `wal_anchor_count` — how many per-core WAL anchors the database carries (`docs/spec/wal.md` §14-3) — is printed **only under `per-core`**, because under one stream it can only ever say 1. `core` is the core serving **this session**: under `peer_listeners = on` the kernel picks the accepting core and a client cannot choose it, so this is how a client learns which relations it can write; constant for the session's life. `0` on every session when peer listeners are off. **On a peer** the line also carries its lease refills' cost: for each of `extent`, `trxid`, `rowid` — `<kind>_refill_requests=<n> <kind>_refill_grants=<n> <kind>_refill_wait_max_us=<n> <kind>_refill_submit_lag_max_us=<n> <kind>_refill_grant_lag_max_us=<n> <kind>_refill_resume_lag_max_us=<n> <kind>_refill_submit_lag_max_iters=<n> <kind>_refill_grant_lag_max_iters=<n> <kind>_refill_resume_lag_max_iters=<n>`: requests sent and grants received, and the longest wait from the request's submit to its completion, split into submit→sent (the reactor queueing the request task), sent→grant received (the ring and core 0), and grant received→the parked coroutine resuming (this core's reactor). Absent on core 0, which leases from nobody. |
...
| `SHOW META`'s recovery block | none | `recovery_records=<n> recovery_committed=<n> recovery_rolled_back=<n> recovery_compensations=<n> recovery_redo_applied=<n> recovery_pages_healed=<n> recovery_torn_tail=<0\|1>`, then `recovery_analysis_us=<n> recovery_redo_us=<n> recovery_high_water_us=<n> recovery_undo_us=<n> recovery_checkpoint_us=<n>`, then `recovery_relations_checked=<n> recovery_relations_missing_pages=<n> catalog_recovered=1 recovery_assertions_enforcing=<n> recovery_assertions_unrecovered=<n> recovery_assertions_foreign=<n>` | What the **last mount's recovery** did (`docs/spec/wal.md` §13). Appended to `SHOW META` by a server; **absent entirely** in an embedded caller that mounts nothing, because a dispatcher with no report has no answer and printing zeroes would be one. **Under `wal_topology=single` a peer runs no recovery pass**: core 0 recovered the one log before any peer was built (`docs/spec/wal.md` §3), so the block carries the leading field `recovery_by=core0` and every scan counter under it — `recovery_records` through `recovery_torn_tail` — is 0 because nothing was scanned here, not because nothing was found. The `recovery_assertions_*` three and `recovery_checkpoint_us` are still *this* core's: it resumes its own relations' assertions and writes its own completion checkpoint. `recovery_by` is **absent** wherever the numbers are the reporting core's own — on core 0 always, and on every core of a `per-core` volume, where the block describes *that core's* mount of its own stream, scanned from the anchor core 0 holds for it, so after a clean stop every core's `recovery_records` is small, not only core 0's. `recovery_records` is what the scan read — bounded by the previous mount's completion checkpoint, so a large log does not mean a large number. `recovery_torn_tail=1` is the **expected** shape of a crash, not a fault. The five `_us` fields are omitted when the mount supplied no clock, for the same reason: a duration of zero and an unmeasured duration are the same bytes. `recovery_relations_missing_pages` counts user relations the catalog still describes whose descriptor or var-heap root page the crash took. `catalog_recovered=1`: catalog writes are logged and redone like any page. `recovery_assertions_enforcing` is how many surviving assertions the mount could resume enforcing; `recovery_assertions_unrecovered` is the honest remainder — a declaration whose directory could not be rebuilt is **left out of the registry**, so `SHOW ASSERTIONS` reports `enforcing=0` for it rather than a constraint that would admit every write, **and on a relation this core owns its writes are refused too** rather than admitted unchecked. `recovery_assertions_foreign` is neither: an assertion is enforced by the core that owns its relation, because that core's writes are what maintain its Bound Cabin, so on every other core it is counted here and skipped — a correctly-partitioned instance reports a non-zero `_foreign` on each core and is not half-broken. `SHOW ASSERTIONS` carries an `enforced_by_core=<n>` field on a row whose relation another core owns: `enforcing=` there is *this* core's answer, and the named core is the one that can answer for the instance. |
| `SHOW META`'s cross-core write counters | none | `cross_core_write_refusals=<n> cross_core_write_refusal_keys=<n>`, and when a key exists `cross_core_write_refusal_detail=<home>><target>:<oid>=<n>[,...][,+<n>more]` | The writes this core refused because the relation belongs to another core, or because the transaction was already bound to one (`docs/spec/crosscore.md` §6). An autocommit single-relation statement is *shipped* to the owner instead of refused, so what is counted here is what shipping does not convert - a statement inside a transaction, and one spanning two owners. The converted population is `shipped_statements` in the row below. **Printed unconditionally, zero included** - unlike the blocks above it, a zero is an answer (*this workload asked for no cross-core write*). The counter is **core-local**, so a whole-instance total is one reading per core, and the unit is a **statement**, not a row: a refused multi-row `INSERT` counts once. `_keys` is how many distinct (home core, target core, relation) triples appear; `_detail` lists them in key order (stable run to run), capped at 16 with `,+<n>more` when it truncated - a silent cut would read as "these were all of them". **What it cannot see**, because the key needs a resolved relation: DDL on a peer (refused by verb before any relation is parsed) and anything refused before resolution at all. The two owner-core refusals, `RelationWriteRightsPending` and `IndexBuildPending`, are excluded **by decision** - the write is this core's own, waiting on a grant or a build window, not a cross-core write. |
| `SHOW META`'s statement-shipping counters | none | `shipped_statements=<n> shipped_replies=<n> shipped_refusals=<n> shipped_wait_us_max=<n> shipped_waiting=<n> shipped_late_executed=<n> shipped_identity_mismatches=<n>` on a core that can ship, and `shipped_executed=<n> shipped_running=<n> shipped_deduped=<n> shipped_unanswerable=<n> shipped_early_evictions=<n> shipped_enrolled=<n> shipped_enrolments=<n> shipped_enrolment_refusals=<n> shipped_enrolment_expiries=<n>` on a core that can be shipped to | **Absent, not zeroed, where shipping is not armed** - a single-core instance prints none of them, which is the honest reading of "there is nothing to ship". The first group is this core as an *arrival core*: statements it sent to owners, answers that came back, how many of those were refusals, and the longest a statement stayed parked waiting. The second is this core as an *owner*: statements it ran for other cores, how many are running now, duplicates it answered from its record rather than running again, and duplicates whose outcome it could no longer state. `shipped_statements - shipped_replies` is "still parked, or lost"; `shipped_identity_mismatches` and `shipped_early_evictions` should both be **0** and are printed so that can be checked rather than assumed. The four `shipped_enrolment*` / `shipped_enrolled` fields are this core as a **participant** in a cross-owner transaction (`docs/spec/cross-owner-txn.md`). `shipped_enrolled` is how many it is holding right now, and each one is a live local transaction pinning this core's read horizon and holding one of its 64 live-transaction slots - so it is a number with a cost, not a rate. `shipped_enrolment_refusals` counts enrolments this core declined, its own `kShippedMaxEnrolled` limit or a transaction-id lease it could not draw; both are retryable and a coordinator may act on them. `shipped_enrolment_expiries` counts transactions the **lifetime** ceiling rolled back — 60 s from the context's own `BEGIN`, whether or not statements were still arriving (`docs/spec/txn.md` §1). It was a "should be 0" defect counter while the ceiling measured idleness; since AN-R14 an abandoned context and a healthy transaction that simply ran past 60 s both land here, and nothing in the counter separates them. `shipped_join_refusals` is a **subset of `shipped_enrolment_refusals` and not a count beside it** - do not sum them: a statement that could only join a context and found none gone, refused retryably instead of silently opening a second transaction for the same one. `txn_watermark_refusals`, printed on the arrival-core side and **only when non-zero**, counts transactions refused because a participant answered from a different snapshot than the one they had been reading it at. Core-local, like every counter here: one reading per core. |
| `SHOW META`'s group-accounting block | none | `sched_wall_us=<n> sched_iterations=<n> sched_idle_blocks=<n> sched_wake_race_skips=<n> sched_parked_idle_blocks=<n> sched_idle_block_us=<n> sched_wakes_sent=<n> sched_wakes_received=<n> sched_spurious_wakes=<n>`, then for each of `foreground`, `maintenance`, `system`: `sched_<group>_polled_us=<n> sched_<group>_polls=<n> sched_<group>_consumed_us=<n>` | What this core's reactor spent, and on whom (`docs/spec/sched.md` §4). `sched_wall_us` is wall time since the reactor's **first iteration** (0 before it runs), so `sched_wall_us - sum(sched_*_polled_us)` is the reactor time charged to **no** scheduling group: the WAL drain's `fdatasync`, the idle block in `PollReady`, timer callbacks, the io drain. **Two counters per group and the difference matters**: `_polled_us`/`_polls` are cumulative and never decay, while `_consumed_us` is the share law's own input and is *halved periodically* so history does not dominate the pick - a scheduling weight, never a total. A spin reads as `_polls` climbing while `_polled_us` does not. **Absent entirely** off a reactor (an embedded caller, a socket-free test), the rule the recovery block follows: a dispatcher with no reactor has no answer, and zeroes would be one. The same block carries the idle policy's three counters (`docs/spec/sched.md` §7): `sched_idle_blocks` is how often this reactor slept with its wake flag raised, `sched_wake_race_skips` how often the pre-block re-check found a message a sender had decided not to wake for - the race the flag exists for, so a run holding it at 0 has not exercised it - and `sched_parked_idle_blocks` the blocks taken while tasks were still queued, every one of which was a spin before a parked coroutine stopped counting as runnable. **Reading the first two against `sched_iterations` is how a wake path is checked from outside a process.** Four more make the wall clock add up: `sched_idle_block_us` is the time this reactor spent inside a `PollReady` it was allowed to block in - its sleep - so `sched_wall_us - sum(sched_*_polled_us) - sched_idle_block_us` is the time charged to nobody that was **not** sleep. `sched_wakes_sent` is the whole **instance's** wake count and therefore repeats identically on every core - it equals the sum of the cores' `sched_wakes_received`, which is the consistency check - and `sched_spurious_wakes` counts wakes this reactor read whose iteration then drained no message, which the send/sleep race makes ordinary rather than wrong. |
| `SHOW META`'s group-commit batch | none | `wal_group_commits=<n> wal_group_batches=<n> wal_mean_group_batch=<f.fff> wal_group_batch_max=<n> wal_group_batch_min=<n>` | **How many commits this core's fsyncs are amortised over.** D2's whole mechanism is that one sync resolves a batch, and `kds.conf.sample` states the edge that makes it matter: *"a batch of one is a batch"*. `wal_group_commits` is D2 commits registered on this core, `wal_group_batches` the syncs that resolved at least one, and the quotient is the mean batch. **`1.000` is the cliff** - every commit paid its own device sync, which is `strict` wearing `group`'s name. Measured against the `bench/` tree at `1769487`, the batch is about `n/2` from four concurrent committers on a core upward and 1.000 below, on a peer and on core 0 alike, so a core with one or two committing sessions pays `strict`'s device cost under `group`'s name. `wal_group_batch_max` / `wal_group_batch_min` are the spread beside the mean: `min` is over non-empty batches, so `min == max` is a batch that never varied and `0` means none has completed. `0.000` means **no batch has completed**, which is a different fact from a batch of one and prints differently: it is what `relaxed` shows, since D3 registers no group commit at all. Core-local and cumulative from mount, like the sync counters below, so a reading is a before/after delta. A shipped write commits on its relation's *owner*, so reading the core that actually paid needs a session there (`peer_listeners = on`): a session accepted on core `c` gets core `c`'s counters back. **Absent** wherever the sync counters are. |
| `SHOW META`'s WAL sync counters | none | `wal_syncs=<n> wal_interval_syncs=<n> wal_sync_failures=<n>` | What this core's log cost the **device**. Core-local, like every counter here, and **cumulative from mount** - including recovery's own syncs - so a reading is a before/after delta, never a total. `wal_syncs` is every device sync this core performed, from both places that perform one: `WalManager::Sync()` on the reactor, which is every sync somebody is **parked** on (a commit under `strict` or `group`, a cross-owner prepare, a client `SYNC`, the checkpoint gate), and the WAL writer thread, which takes D3's loss-window tick alone. **`wal_syncs - wal_interval_syncs` is the part somebody was parked on**, and that rule holds on every core. **Under `wal_topology=single` a peer's `wal_syncs` and `wal_interval_syncs` are structurally 0, and that is the truth rather than a gap**: it performs no device sync at all, flushing through core 0's stream and waiting on core 0's writer, so every `fdatasync` the instance pays is counted on core 0 and counted once — and its interval count is 0 because the loss window belongs to the log, which has one drain bounding it. **A peer's durability cost is read on core 0.** `wal_sync_failures` is **not** structurally 0 on a peer and is the one exception: its flush of the shared ring can fail, and so can its wait on core 0's writer, and both are counted here — a failed wait being counted on core 0 as well, which makes this the one field where two cores report one event. Under `per-core`, `wal_interval_syncs` counts the loss-window tick whether it was handed to the writer thread (core 0) or taken inline because there is no writer thread (every peer core, which starts none). On core 0 the two differ by the sync in flight and by any pair the writer coalesced, since the tick is counted where it is *requested* and the writer syncs where it *completes*. A sync is a **durability point, not a syscall**: one increment issues one `fdatasync` per open segment, so an N-segment log costs N syscalls per count, and the two fsyncs a *new segment* costs are in neither counter. A cross-owner transaction under `group` performs three device syncs and *waits* for two of them (`docs/spec/cross-owner-txn.md` §2): `wal_syncs` counts syncs performed, never syncs waited on. `wal_sync_failures` should be 0; it also counts a failed **flush**, so a non-zero value is not proof a sync was attempted. **Absent** where the dispatcher has no WAL at all - an embedded caller, never a served core. |
| `SHOW META`'s WAL ring counters | none | `wal_ring_full=<n> wal_ring_full_refusals=<n>` | **The append path's backpressure**, and the pair to read when write throughput is below the device's. `wal_ring_full` counts the times an append found the ring full and drained it inline: the appending statement paid a synchronous flush on the reactor thread and then got through. Under `wal_topology=single` every core appends into the **same** ring, so a stall was caused by the instance's producers together but was *paid* by this core - sum the cores for the instance's stalls, read the spread to see whether one core is starving. It is counted per drain attempt rather than per append, because a drain this core performs can be consumed by another core before this one gets the latch back, so the number is stalls paid and sits above the number of appends that stalled. `wal_ring_full_refusals` is a **different and much worse event**: an append that exhausted its drain attempts and was refused `OutOfSpace`, so a statement failed rather than waited. Zero is the expected reading at every load; nonzero is a ring sized below the workload, not a slow device. **And zero is the only reading any test has produced**: reaching it needs an append to lose the drained space to another core four times running, which cannot be staged from one thread, so the counter is proved to stay at zero where it must and is unproved where it fires (`tests/wal_manager_test.cpp`). Core-local and cumulative from mount, like every counter here. **Absent** wherever the sync counters are. |
...
| `SET CABIN_OPTIMIZER [=] <v>` | `ON` or `OFF` | `OK cabin_optimizer=<on\|off>` | The runtime kill switch for the cabin optimizer (`docs/spec/physical-optimizer.md` Part II). Non-destructive in both directions: `OFF` halts new decisions and in-flight builds and touches no existing Cabin. Server-wide, not per connection — the controller is per core, not per session. `SHOW META` reports the current value; `SHOW CABIN_OPTIMIZER` shows what the controller is managing and what the executor has applied. |
...
verb with a named target list, so `DROP EVERYTHING` answers `ERR only DROP
TABLE, DROP CABIN, DROP INDEX and DROP ASSERTION are supported` rather than
`ERR unknown command` - naming what DROP does take is more use than a
generic refusal.

`SHOW PAGE`'s reply is a one-off exception worth calling out: on the debug
port the wire contract still holds (exactly one line, no raw newline
byte), but its sections are joined with the literal two-character escape
`\n` (backslash followed by `n`, not an actual newline byte) so the reply
can render as a multi-line, human-readable dump on the client side without
...
The grammar is `src/parser`'s, executed through `command_dispatcher.cpp`
and `src/exec/row_codec.cpp`; `command_dispatcher.hpp`'s doc comment
states exact behavior. **`float`** is the one type refused for its
semantics rather than for want of work.

**The primary key is a unique 40-bit id carried by the Keystone word, never
rebound and never updatable** (`CLAUDE.md` invariant 11). Column 0 of every
relation is that id; an `INSERT` names it or omits it per row, and an
omitted one comes back in the reply as `id=<n>`. `UPDATE` cannot change
it. Two rows with the same key are not expressible, which is the point -
a tuple's id is its identity, and the clustered index addresses tuples by
it directly.
...
flattened to a single line before it goes out. A file with **no `;`
anywhere** is read as one statement per line instead, which is what
`adhoc/*.sql` uses. `--` starts a comment to end of line, except inside a
quoted string, and SQL's doubled `''` escape is honoured.
...
and `UPDATE`. It is deliberately single-connection. Run the server from a
Release build on a scratch data file, or the numbers describe a `-O0`
build:
...
Since INSERT is logged, the insert phase measures the `durability`
setting as much as the engine, and the two must be quoted together.
`group` matching `strict` on one connection is the expected result, not a
bug: a batch needs concurrent committers. And **do not benchmark on
`tmpfs`** — `fsync` there is free, all three classes come out identical,
and the measurement says nothing.
...
round-trip floor on loopback is tens of microseconds, so a change worth
5 µs of engine time is invisible in qps and obvious in the server-side
column.
...
Three properties shape everything it prints, and are stated in its own
footer rather than left for the reader to remember:
