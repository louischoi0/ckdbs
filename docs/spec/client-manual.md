...
| `isolation` | — | `read committed` | The level a connection starts at, and so the level an autocommit statement runs at (`docs/spec/txn.md` §1). `read committed` takes a read view per **statement**; `repeatable read` takes one per **transaction**. READ COMMITTED is the default for a reason specific to this engine: under first-updater-wins with little waiting, holding one view for a whole transaction turns more concurrent writes into retryable aborts. `serializable` is refused with its reason. This is the server rung of a three-level chain — a connection overrides it with `SET ISOLATION LEVEL`, one transaction with `BEGIN ISOLATION LEVEL`. Names are case-insensitive and accept `-`/`_`. |
...
| `SHOW META` | none | `version=<n> create_time=<n> last_mount_time=<n> wal_topology=single cabin_optimizer=<on\|off> core=<n>` plus the blocks below | Dumps the superblock. Times are Unix seconds. `wal_topology` is what this volume's log **is**, and it always reads `single` — one stream for the instance. A volume written with one stream per core is refused at mount, so no other value can be printed; the field stays because it is a durable format fact a client may want confirmed. `wal_anchor_count` used to appear beside it under `per-core` only, since under one stream it can only ever say 1; that is now every volume, so it is not printed. `core` is the core serving **this session**: under `peer_listeners = on` the kernel picks the accepting core and a client cannot choose it, so this is how a client learns which relations it can write; constant for the session's life. `0` on every session when peer listeners are off. **On a peer** the line also carries its lease refills' cost: for each of `trxid` and `rowid` — the `extent` kind went with the extent lease at AW-S1b, when one buffer pool and one free map left nothing to lease — `<kind>_refill_requests=<n> <kind>_refill_grants=<n> <kind>_refill_wait_max_us=<n> <kind>_refill_submit_lag_max_us=<n> <kind>_refill_grant_lag_max_us=<n> <kind>_refill_resume_lag_max_us=<n> <kind>_refill_submit_lag_max_iters=<n> <kind>_refill_grant_lag_max_iters=<n> <kind>_refill_resume_lag_max_iters=<n>`: requests sent and grants received, and the longest wait from the request's submit to its completion, split into submit→sent (the reactor queueing the request task), sent→grant received (the ring and core 0), and grant received→the parked coroutine resuming (this core's reactor). Absent on core 0, which leases from nobody. |
