...
- **Extent = 64 pages** (`kDefaultExtentPages`) — the unit of file growth. It was also the unit of the per-core extent lease, which AW-S1b struck: a core that could not reach the free map reserved a run of ids up front, and every core reaches the one map under its own latch now (below).
...
- **The map has a latch of its own** (AM-S3), `map_latch_`, armed on the same word as the frame table's. It guards `map_regions_` and every region in it — the two bitmaps, the lazily installed headerless pointer (FM6), and the dirty flag. Three rules, and each is a fix for something that was not merely unsynchronised but undefined:
  - **Every public reader takes it.** `IsAllocated`, `IsHeaderless` and `map_residency` did not, and `IsAllocated` sits on the fault path, the writeback path and the WAL gate — so the engine's most frequent map read was a `std::map::find` racing region creation's `insert`. Internal callers already holding it use the `...Locked` bodies.
  - **Device work never runs under it**, the rule this class has had since AM-S2 2b. Region creation resolves the file grow and the device read *outside* the hold and publishes with `try_emplace` *under* it, so two cores creating one region both succeed and the loser returns the winner's bitmap rather than replacing a map the winner has already allocated out of. `FlushMaps` copies each dirty region's bytes under the hold, clears its flag there, and writes after — clearing at the copy rather than after the write is what makes a concurrent allocation re-dirty the region instead of losing its bit.
  - **The order is the frame table's latch and then this one, never the reverse.** Both halves are debug-asserted rather than stated: a public reader aborts if reached from inside the map hold, and the frame table's one structural mutation aborts if reached while holding the map.
- **Why a second latch and not the frame table's.** The two are *nested*: `FetchPinned`'s hit path holds the frame table across `Resolve`, and `Resolve` begins with `IsAllocated`. One latch would be a second acquisition on one thread, which `base/latch.hpp` says is a hang — and is how AM-S3's census found this, by hanging rather than aborting. It is AM-S2's review's own ruling, *structure → allocation, allocation the inner leaf*.
- **A claimed id is in use before its frame exists.** `CreateAt` sets the map bit under the hold and inserts the frame after releasing it, so between the two the page is allocated, not resident, and its bytes on the device are still zeros — which is the signature the claim path reads as a *torn creation* and re-claims. A second caller naming that id in the window was therefore handed a page the first already had. The store keeps a set of ids claimed and not yet resident, the same shape the fault path's in-flight set uses, and the claim path refuses one that is in it. Found by AM-S3's own cell rather than by its survey, and **pre-existing**: the window does not depend on which latch guards the claim.
- **Who writes the map back when one store serves every core** is answered by the hold rather than by a rule: N checkpointers each call `FlushMaps`, the first to take it clears the dirty flags while it copies, and the rest find nothing and write nothing.
...
- **One `DevicePageStore` for the instance, since AM-S2 step 3** (`2663001`): a peer borrows core 0's store rather than opening its own (`include/kds/server/core_runtime.hpp`), so one frame table caches every core's pages and a page faulted on one core is served from the frame another loaded. This section read *Per-Core Buffer Pools* until 2026-09-06, and the sentence it opened with — "multi-core adds instances, not synchronization" — is the thing that stopped being true.
- **Unconditional since AM-S4(d).** The sharing was wired only where the volume had one WAL stream, because the writeback gate is a property of the log; a volume with more than one is refused at mount now (`wal.md`, the superblock's `log_topology`), so every mountable volume has one pool.
...
- **The page latch** (`include/kds/storage/page_latch.hpp`; `rules.md` §3's row): every resident frame carries one 32-bit word — an exclusive bit, the owning core, a count — operated on by CAS through `std::atomic_ref`, and **armed only at `cores > 1`** from the superblock's core count; an unarmed store never touches it. Taken with the pin and released with it: `Get` and the `Create*` accessors hold a frame exclusive, `GetForRead` shared. **Writeback takes it too since AT-S8 step 1b**: each page is copied under its latch, shared - waiting out another core's exclusive holder on a flush, trying once and leaving the frame dirty in the background drain - one page at a time and released before the WAL gate; and a frame is cleaned after its write only if its dirty generation, bumped at every dirty mark, has not moved since the copy. Before it, a writeback copied under the structure latch alone and cleaned unconditionally, so it could persist half of another core's write with a valid checksum and drop the mark of a write that had not gone out. **And a writeback claims each frame from its copy to its clean since AT-S10e** (§8), so two writebacks of one page land in copy order; the claim is taken before the page latch, and its waiter holds no claim and - by the premise `HeldFrames::kWait` states and nothing enforces, that no flush caller holds a page latch across the flush - no page latch. Re-entrant for the owning **core** — a task may hold one page twice, and shared under its own exclusive — where "the owning core is the running task" is a discipline, not a gate: no task parks holding a pin, the suspend audit *records* a violation in debug builds only (`sched/coro.hpp`'s `NoteSuspension`, not fatal), and a park under a latch would be a silent second exclusive grant to the next task on that core; a per-task owner is AM-S2's escalation if the audit ever records one. **Never upgraded**: a shared holder asking for exclusive is a self-deadlock, aborted in debug and an unbounded spin on the reactor thread in release. The debug check **stopped being a proxy at AM-S2 step 3b**: it read the store's own `pins` as "the shares are mine", which one table serving every core turns into "two cores hold one pin each" as readily as "this core holds two" — a detector that would fire on correct traffic and miss the defect it exists for. It is a thread-local multiset of the pages this thread holds shared now, exact on the same premise `CurrentCore()` rests on: one reactor per thread, no handle crossing threads. Acquisition order: **outer** to the WAL stream latch — a task appends while holding its page latches, and no WAL path asks for a page latch while holding the stream latch (recovery's redo is the one place `wal/` touches a frame at all, and it runs on the mount thread holding no stream latch) — never nested with the visibility window latch, never held across a park; on the fault path a task may hold other frames latched while a writeback it triggered waits on the WAL gate, which is sound (the writer thread takes no page latch) and a latency cost AM-S3 prices. Page against page is **unordered, and the reason it was safe is gone**. It read: two page latches are held on every split and chain growth (a descent holds one at a time), and one core owns its pool, so no two holders can wait on each other. The second clause stopped being true at `2663001`. The pairs held at once are listed in `device_page_store.hpp`'s "The page latch" section — the split's old leaf and new sibling, chain growth's old tail and new page, and since AM-R8 a scan ring's shared hold on a heap leaf across its consumer's var-heap fetches — and **stating the order is still owed**. Two more items are owed with it. A re-validation after the re-fetch the two S-then-X fixes in `btree.cpp` and `index_tree.cpp` introduced: each decides on a read handle and acts after dropping it, which was sound only while one core owned its pool. **The clustered tree's is closed since AT-S5c** (`LeafStillCoversKey`, and a stale descent restarts), **and the secondary index's since AT-S15** (the same check, its bound the right sibling's first entry, which holds only while no index entry is removed - `index.md` IX14); a split's walk back up the recorded path is unchecked in both trees (on the index, since AT-S15, as a key range whose inserts are refused on every attempt rather than a lost row) (the clustered one reaches it only through a caller-named key's divide), a quiet wrong answer when two cores write one tree (`docs/inflight/bugs/a-secondary-index-descent-is-not-revalidated-across-cores.md`, `docs/inflight/bugs/a-btree-divide-can-promote-into-a-parent-another-core-divided.md`). And starvation — waits spin, then yield, with no queue and no writer preference, so steady shared holders of a hot page can starve an exclusive request; that one is not made worse by sharing, only reachable by more threads. **The latch is live in production at `cores > 1`** and inert at one core, where it is never armed; the primitive, its order and its cells are AM-S1's, and `device_page_store.hpp`'s header carries the same statement.
- **Cross-core page access is what the pool now is**, for writes as well as reads. It did not exist through AM-S1 — work moved to the owning core over the message interface — and a peer reading a page core 0 faulted is the cell AM-S2 step 3 landed on. The *write* kept moving to the owning core, by statement dispatch, until AT-S5: `MayWrite`'s last arm (the system range, AW-S1b having taken the extent lease, the write grants and the stamp claim) went there, and **every core writes every page, the superblock and the catalog pages included**, under the page latch on the core its statement runs on; the free map, which lives in the store's regions rather than in frames, is written by every core under its own map latch (§5; `crosscore.md` CC11 names each).
- Ownership: **none, since AT-S9** (AR0-5 D17). A relation's pages belonged to the core `sys.tables.owner_core` named; that column's bytes are reserved now - written 0, never read (`docs/spec/crosscore.md` CC7). A `PAGE_HANDOFF` record is written by nothing since AT-S9 retired the range allocator that was its last appender, and read by nothing since AM-S4(d) (`docs/inflight/known-gaps.md`, WAL). M0 removed the second stream, M1 the second pool, and AT the second owner.
- **The device under them is shared, and that is declared** (`rules.md` §3): one `PageDevice` serves every core's store, and **any core grows it** (the operator's decision of 2026-09-08). Core 0 alone grew it while each core had its own store and its own extent lease; a peer allocating through the instance's free map has grown it since the pool became one, and from two more paths since AW-S1b. What synchronises that: `page_capacity_` is a `std::atomic<uint32_t>` — a reader loads a bound that only rises, so a stale value under-reads and faults nothing it should not, and the class would not survive a shrink — and the grow is serialised inside the device (`FilePageDevice::grow_mu_` around the `posix_fallocate`, the new bound published after the file has the blocks; `MemoryPageDevice`'s one latch), so two cores running off the end at once produce one grow. The growth lock is a leaf — nothing is taken under it — and is never held across a park. **A page latch is outer to it**: what reaches the grow is a split or a chain growth, which holds its page exclusive across `CreateNew()`. Neither of this store's own latches is held there: the structure latch drops across the device read (the "what synchronizes it" bullet above) and §5's "device work never runs under it" is the map latch's rule. The order is the page latch, then this, and nothing under it.
...
- **A writeback holds each frame from its copy to its clean** (AT-S10e, `Frame::writing` in `device_page_store.hpp`, under the structure latch, in the frame's padding so `Frame` stays 40 bytes). The claim is set when a run is found, and released at the clean — cleaned or not — under the same hold, and on every early exit (the drain's truncated run, a WAL gate refusal, a failed device write). A writeback that meets a claimed dirty frame either waits for the claim and looks again (`HeldFrames::kWait` — `Flush`, `Sync`, `FlushPages`, so every checkpoint: still dirty means a write the other did not carry, and this one writes it) or leaves the frame to its writer (`kSkip`, the dirty-eviction-queue drain); a run stops at a claimed neighbour. The wait (`AwaitWritebackClaim`: brief yields, then 50 µs sleeps, because a claim spans a device write and possibly a WAL sync) holds no claim of its own — a run is claimed, written and released before the next is looked at — so two writebacks cannot wait on each other. Before AT-S10e the dirty generation made the clean honest but left the order of two writebacks' device writes to chance: the one that copied the older image could land after the newer one had written and cleaned the frame, leaving the disk one write behind a clean frame — a committed update lost with no crash.
...
- Config: `buffer_pool_frames` (an undivided instance total since AM-S2 step 3, 0 = unbounded; it read "divided across cores, a value below `cores` is refused at startup", which is what a per-core-stream volume still runs), the device's extent size (§5), checkpoint cadence (owned by wal.md), resident-class list (build-time).
...
- **Hit path:** a probe of the frame table (`std::unordered_map<PageId, Frame>`), a pin increment and a usage-counter bump — no syscalls. **Not lock-free since AM-S2 step 3**: the probe and the pin are one hold of the structure latch, and the page latch word is taken after it. Both are armed only at `cores > 1`, so the "no locks, no atomics" this line used to claim is still exactly what a single-core instance pays.
