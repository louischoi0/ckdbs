...
- **Extent = 64 pages** (`kDefaultExtentPages`) — the unit of file growth. It was also the unit of the per-core extent lease, which AW-S1b struck: a core that could not reach the free map reserved a run of ids up front, and every core reaches the one map under the store's structure latch now.
