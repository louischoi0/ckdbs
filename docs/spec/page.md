...
- **Conditional on the log, not on the core count.** The sharing is wired only where the volume has one WAL stream (`Expeditor`, `single_stream()`), because the writeback gate is a property of the log; a pre-M0 volume still mounts a store per core and every claim this section makes about sharing is inert there. Both shapes are live, and which one an instance is in is a superblock fact.
...
- Ownership: **none, since AT-S9** (AR0-5 D17). A relation's pages belonged to the core `sys.tables.owner_core` named; that column's bytes are reserved now - written 0, never read - and every core writes every page above the system range under the page latch, on the core its statement runs on (`docs/spec/crosscore.md` CC7). A `PAGE_HANDOFF` record is written by nothing since AT-S9 retired the range allocator that was its last appender, and read by nothing since AM-S4(d) (`docs/inflight/known-gaps.md`, WAL). M0 removed the second stream, M1 the second pool, and AT the second owner.
