...
- **The ring holds a shared pin, and the page latch, on exactly the page
  ...
  frame** (AM-R8, 2026-09-05). That is what makes "the span is valid until
  ...
  The residency bound stays exact and moves by one. A slot rotates when —
  and only when — the fetch that took it *faulted*, and the fetch itself
  reports that rather than a residency probe taken before it, which on a
  shared pool would be stale before it was acted on. The release therefore
  follows the fault instead of preceding it, so residency grows by at most
  the ring's size, plus one frame for the length of a `Fetch` that faults.
...
- Consumers, as the tree has them: the Cabin optimizer's build walk
  ...
  builder and aggregate full scans" until 2026-09-05; neither has ever
  opened a ring.
