...
- The row encoding above is `include/kds/wire/row_codec.hpp`. It knows about neither frames nor cores, and it is the engine's one row encoding (`docs/spec/crosscore.md` CC2): its consumers are the session's result batches and the load stream's chunks (`bulkinsert.md` BI6). Cross-core `STEP_BATCH` payloads were a second consumer until AT-S10 deleted the remote-step protocol. `FLOAT64` has a wire format and no storage — nothing can store one, and the encoder refuses what storage refuses. A decimal value whose scale disagrees with its column is refused at encode, never rescaled — the same rule the storage codec applies.
...
**What suspension means, and what it does not.** This engine has no suspension point at a row boundary: a walk holds a page pin across every row of a page, and since AT-S10 no walk parks at all - the page-boundary park under the cross-core gate (`resume_gate_`) was the executor's one suspension point, and no caller passes a gate since the remote-step protocol was deleted. So **`max_rows` bounds delivery, not execution** — the statement runs whole into the portal's buffered batches, and `max_rows` decides how many rows leave now. Three consequences:
...
- **Portal-idle timeout: 60 s**, on the injected clock, behind `kPortalIdleTimeoutNs`. The derivation is a reuse rather than a new number: this tree spent 60 s on the two waits whose event was human-or-network-scale (the index and assertion builds' reply deadlines, until their ships went at AT-S5e and AT-S5d) against 10 s for the machine-scale ones, and an idle portal waits on a client.
...
- **The state machine is `include/kds/server/kwp_session.hpp`.** It binds to the real `CommandDispatcher`, and `kds::wire` sits *below* `kds::server` (the row codec is in `wire`, below the server, where the session and the load server both reach it), so a dispatcher-dependent state machine in `wire` would invert that arrow. What lives in `wire` is what has no engine dependency: the frame codec, the handshake, the error registry.
- **The engine's result rows reach the protocol through one seam**, `server/result_sink.hpp`. The three emission points (the local walk, the sorted drain, the aggregate fold; the cross-core fan-in was a fourth until AT-S9 retired it) call a sink: the text form is one implementation and the wire form is the other, and there is no third copy to forget a `type_val`. The sink lives on the `Session` and not on the dispatcher, because a statement can *park* and another connection's statement then runs on the same dispatcher.
