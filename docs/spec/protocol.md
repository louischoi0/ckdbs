...
- **Portal-idle timeout: 60 s**, on the injected clock, behind `kPortalIdleTimeoutNs`. The derivation is a reuse rather than a new number: this tree spent 60 s on the two waits whose event is human-or-network-scale (`kIndexBuildReplyDeadlineNs`, and the assertion build's until its ship went at AT-S5d) against 10 s for the machine-scale ones, and an idle portal waits on a client.
