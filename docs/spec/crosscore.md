...
one stream, so no cross-core pair of views can split them.
...
- A write to a range this core does not own, reaching `CheckWriteAffinity`
  ...
  single-stream, and nothing 2PC-shaped is in them (§5's shared statement
