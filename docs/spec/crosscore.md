...
- A non-pk *read* predicate names none; the default is every range of
  ...
  `docs/spec/feat-index.md` §13); Cabin answers "which range holds value V"
  for an observed key under its own banked-authority rules
  (`docs/spec/feat-cabin.md` §4a, §6a); a Waystone trail names pages, a page
...
- FK (`docs/spec/impl-foreign-keys.md`) stays co-located: RESTRICT validation is a
...
- **Secondary indexes** — per-range local vs global is `[OPEN]`
  (`docs/spec/feat-index.md` §13; reading on record: local per range, cut by
...
- **Cabin** — entry sets are memory-resident and observed per core, and
  ...
  story belongs to `docs/spec/feat-cabin.md` §11. Gate: a cabined relation
...
- **Foreign keys** — gate: an FK parent or child does not split
  (`docs/spec/impl-foreign-keys.md`; the §6 bullet above says why).
...
11. **Migration ordering:** a crash injected between each pair of
   ...
   `spec-page-lsn-cross-stream.md` §9, and a reachable stamp mismatch
...
- Auxiliary placement under a split relation — each lifts its §6a gate:
  per-range local vs global secondary indexes (`docs/spec/feat-index.md`
  §13), Cabin (`docs/spec/feat-cabin.md` §11), var-heap partition
  (`docs/spec/heap-and-tuple.md`), FK (`docs/spec/impl-foreign-keys.md`).
