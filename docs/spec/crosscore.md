...
| CC10 | Range split & migration | **Split at a page boundary only** — `min_key` is the split key, so no page is ever divided and invariants 2/3 hold by construction; §6b's block-aligned insert split satisfies this vacuously, because the new range starts as its own empty sub-structure (CC8) and no existing page straddles it. **Migration**: the page-level contract is the ratified PL-B handoff (`page-lsn-cross-stream.md` §9); the range-level sequence *around* it is this spec's, and its ordering is a correctness statement — (0) the outgoing owner quiesces the range (in-flight stages finish or cancel; new plans against it surface as §5's stale retryable step error), (1) flush, (2) durable handoff record, (3) **durable directory row before any grant** — the row is a catalog write in core 0's stream, synced before step 4 — (4) grant, (5) invalidation broadcast; the incoming core's first write stamps its stream per PL-C. **A crash before step 4 aborts the migration to the outgoing owner at mount**, sound precisely because the grant is last: the incoming core has written nothing. PL §9 governs the record/redo/stamp halves; steps 0, 3 and 5 are outside its five rules and are owned here, not cited to it. Advisory-reference retirement is priced, not free: a Waystone trail replayed against a moved range misses on the epoch/owner check and falls through, but bumping `relayout_epoch` costs one write per moved page — whether the bump or the owner check alone retires stale trails is R5's to settle against the mover's flush-per-move cost. **Cabin does not self-heal**: its hint miss resolves through the pk *on the same core*, and its entry sets are memory-resident where they were observed. **The pre-grant window (SB-R2, 2026-09-01)** is what that costs, and it is one window with two obligations, both landing between step 3 (the durable directory row) and step 4 (the grant), because step 4 is the instant a second core may write: (a) the **Observational discard** — the relation's entry sets are dropped, since a set banked while the relation was whole was banked under the whole-relation claim and nothing in a set records which claim it was made under (`cabin.md` §4b); and (b) the **Bound** directory load, SA-T7's, unchanged. Ordered *before* the grant and not after: a discard that ran after the boundary was published would be dropping sets that were already subsets. **This holds for migration too** — a migration is a grant, the origin core's owned-range set goes to zero, and a set claiming ranges its core no longer owns is exactly what the discard exists to prevent. What performs the discard today is a direct core-local `CabinStore::Forget` on core 0, which is stronger than a broadcast (no window at all, the discard and the grant being one task on one reactor) and was sufficient because core 0 held the only Cabin store in the engine. **Since AK-S2 (2026-09-02, `instructions/v2.8.0/workorder-ak.md`) every core holds a store**, and the discard still reaches core 0's alone — sufficient only because a range never opens under v2.8.0 (AE-3.2, `range_size_ids` off) and because the serve site's scope rule (`cabin.md` §4b) makes a store fall through on a relation not wholly its core's. The day a range opens on a relation whose owner holds a store, this is the acknowledged broadcast SB-R2 describes and the grant waits on the acknowledgement set; the obligation is the ordering, and the mechanism is whatever makes it true. `known-gaps.md` carries it. **Split's Cabin gate is lifted with it (§6a); migration's is not, because the Bound class is still gated** (SB-R5). The mover is the physical optimizer's Part III and inherits Part I's discipline (observe, decide, report through SHOW, enact through named gates); its policy and constants stay `[OPEN]` (§9). Until the owning docs decide auxiliary placement, split is **gated** (§6a) |
...
self-heal — CC10) binds both. **Amended 2026-09-01 (SB-R1):** the Cabin
bullet's split half is lifted and its migration half is not, so the
sentence now reads — the gates below bind split except the Cabin's, which
binds migration alone:
...
- **Cabin** — **the split half of this gate was lifted 2026-09-01**
  ...
  Observational Cabin's entry set is authoritative for **(observed value ×
  the ranges its core owns)**, so a boundary narrows what a set speaks for
  instead of falsifying it (`docs/spec/cabin.md` §4b). Ranges the serving
  core does not own fall through to their own stage, which is the fan-in
  this section's read surface already opens. The sets banked under the
  *old* whole-relation claim are dropped in CC10's pre-grant window, and
  the two halves — the scoping and the discard — landed in one merge,
  because lifting the gate without the scoping converts a refusal into a
  quiet wrong answer. Gate: **an Observational Cabin no longer gates
  split.** It still gates **migrate**, and the **Bound** class still gates
  both (SB-R5; SA-T7 owns the Bound migration, `docs/spec/assertion.md`
  §6.1's owner rule is what a Bound cabin's chain rests on). The gate that
  survives is asked as the assertion arm below, which is where a live
  Bound Cabin is visible; `RangeEligible` no longer carries a `kCabin`
  arm at all.
