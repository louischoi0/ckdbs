...
Fuzzy checkpoints, run as a `system`-group task per core — each core checkpoints its own active transactions, and **the pool it walks is the instance's since AM-S2 step 3**. This read "its own pools"; there is one frame table now (`page.md` §6), and a peer's checkpoint target is that table (`core_runtime.cpp:954`, `checkpoint_target_.emplace(*store_)`), so the dirty set a core checkpoints is the instance's dirty set rather than its own. **What follows from that is not settled here**: every core but 0 gets a checkpointer, gated only on `core_id != 0` and not on the topology, so under one stream N cores walk one table. Redundant rather than wrong — a page written back is clean when the next walk finds it — but *how many* checkpointers a shared pool should have is a question for the stage that owns writeback under sharing (AM-S3), not for this sentence, which says only what the code does. Under one stream the two checkpoint records name their logging core in the record envelope's `flags` byte, since the log now carries every core's, and step 3 publishes into the fold of §3 rather than into a slot of this core's own:
