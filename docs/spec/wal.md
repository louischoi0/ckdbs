...
- **Analysis, redo and the high-water repair run once per log** — on core 0, there being one log (§3) — followed by a completion checkpoint; `SHOW META` prints the recovery block. The catalog recovers too, and a torn catalog page refuses the mount rather than being served corrupt.
...
- **No prepare is ever written since AT-S6, so nothing resolves one.** The rule that stood here - with one log the coordinator's decision is in the same log as the participant's `TXN_PREPARE`, so **absence of a decision is abort** - was about a protocol that is retired (`cross-owner-txn.md`). It had already stopped being reachable before the stage ran: SA-T0 made a participant that wrote nothing skip the record, and AT-S5 left no participant that could write, so the last `TXN_PREPARE` this engine could append was written before that stage. The record type and recovery's arm for it are dead weight rather than a live contract, and §11-3's prepare floor bounds nothing.
- **A page's stream stamp decides nothing.** It records which core last wrote a page and no branch reads it: redo neither refuses a foreign stamp nor restamps what it applies — those two arms were deleted at AM-S4(d) — and `PAGE_INIT` stamps from the core named in the record envelope's `flags` byte rather than from the replaying core. `docs/spec/page.md` §2b is what is left of it, and carries the account of the document that used to own the subject.
- **`PAGE_HANDOFF` does not remove its page from the dirty-page table**, because what would license that removal is a *flush*, and a flush covers one core's page store while with one log the removal would speak for every core's records. It is neither erased nor seeded: a handoff is an ownership fact, not a mutation, so it must not become a page's recLSN either. §12-1 and `wal/analysis.cpp` carry the case.
...
**Per-core streams are gone, and what they were is worth one paragraph.** A pre-M0 volume had one stream per core, matching the shared-nothing ownership the engine had then and the per-core buffer pools `docs/spec/page.md` §6 used to describe — a core logged only mutations to state it owned, with no shared tail pointer on the append path. LSNs were stream-local and never compared across streams; the superblock held one anchor slot per core; recovery ran per core in parallel; a participant's prepared transaction was resolved by opening the *coordinator's* file. **AM-S4(d) refused such a volume** on D14 (no in-place migration; a new major version mounts only volumes it created) and deleted every branch that served it: the peer's own `FileLogDevice` and recovery pass, the per-core anchor slot, `CoordinatorStreamResolver` and the whole cross-stream prepared-resolution phase, redo's foreign-stamp refusal and restamp. Nothing was migrated because nothing *could* be: those LSNs are not comparable, so no rewrite of the topology word could make them one stream's.

- **The core count is pinned, and no reason this spec has given for it is still true.** The superblock records `core_count` at bootstrap and a mount whose running `cores` disagrees is refused, naming both numbers (`superblock.hpp`, `src/bootstrap/bootstrap.cpp`). Two reasons have stood here and both are dead. Under per-core streams it was that a mount at M would leave |N − M| streams with nothing to replay them; AM-S4(d) ended per-core streams. This bullet then said the anchor's warm-up replaces it, "defined over *every core has published*" — but AM-S4(d) also left `SetWalAnchor` refusing every `core_id` but 0, so slot 0 is the only slot, the fold seeds from the lowest populated one, and "every core" is a condition over exactly one core. A changed count neither parks the fold nor advances it past anyone. **The refusal is still in the code and its message still cites per-core streams**, which is a false statement to an operator and a one-line fix nobody has made. Whether the pin stays at all is **AT's** (`ar0-5-amendment-uniformity.md`, M3 "Uniformity"), which retires core specialization; AR2's E9 argued it must stay and was **withdrawn by the operator on 2026-09-08** for arguing from these two dead grounds. Online core-count change is not supported today.
...
- **`core_id` names the stream's owner, not the record's author.** It is 0 on every segment, because core 0 owns the one log and the peers attach to it (§3); a record's own logging core, where it matters, is in the record envelope. Under per-core streams the two coincided, which is why the field still reads as if it were the author's.
...
The tuple header carries exactly **`trx_id` (writer, 48-bit) + `undo_ptr`** — there is **no `xmax` field**. A version's death is the next version's birth: walking the undo chain, the reader already knows the overwriting transaction when it arrives at an older version, so the chain itself encodes validity intervals; storing the boundary twice is redundant. DELETE is a **delete-mark** (slot/Keystone flag) plus the deleter's `trx_id` in the writer field — no separate field. Row locking has **no field of its own**, in the header or on the page: AO-R3 decided no persisted lock bit at all, so the byte stays zero and a row's tuple `X` is the writer stamp the header already carries, with the lock table holding the rest in memory (`txn.md` §5). The Postgres-style secondary role of `xmax` as a lock slot is covered by that stamp. Consequences for WAL: every heap mutation record carries the writer `trx_id` it stamps; undo records carry the *prior* writer id, which is what makes the no-`xmax` reconstruction work. The 48-bit width bounds `txn_id` in §4.2; exhaustion is `OutOfRange`, never wrapped (`docs/spec/txn.md` §4.2).
...
1. A foreground task mutating a page first appends its record(s) into the **WAL ring** — the instance's (§3); preallocated, so an append is a memcpy and a cursor bump, no allocation and no I/O. The **whole** append runs under the stream's latch — the size checks, any segment roll, the encode into the ring and the cursor bump; the reserve/copy/publish split that would have left the copy outside it was tried and abandoned (AL-R1; `docs/spec/page.md` §2b records what became of the document that argued the other way). The frame's `page_lsn` mirror is set to the record's LSN (the on-page header field is stamped by the same mutation).
...
1. **WAL-before-data:** a dirty page may be written to the data file only when the log is durable up to that page's `page_lsn` — the instance's, there being one (§3). **That is what made one buffer pool admissible** (AM-S2 step 3): the gate is a property of the log, so core 0's answers for every core, which is untrue the moment there are two logs. **Enforced inside `BufferPool::Flush` via the `WalDurability` seam** (`docs/spec/page.md` §8); `MarkClean` is reachable only as flush completion. `DevicePageStore` — the store the server runs on — holds the same seam (`SetWalGate()`) and applies it in `Flush`, `Sync` and `FlushPages`, gating on the highest `page_lsn` in the batch. An ungated store flushing a logged page is exactly the violation this rule names.
...
Fuzzy checkpoints, run as a `system`-group task per core — **each core checkpoints its own active transactions, and one checkpoint runs in the instance at a time** (AT-S8). The pool every checkpoint walks is the instance's since AM-S2 step 3 (`page.md` §6), so the dirty set is the instance's; the active-transaction table is not. Each core's `TransactionManager` holds its own live set, unlatched, and recovery's analysis needs every live transaction's undo head in some `CHECKPOINT_BEGIN` inside the replay range — so a single gathered checkpoint would need an instance-wide table, which the operator declined at AT-S8 in favour of this: every core keeps its checkpointer, `wal::CheckpointGate` admits one run at a time and a cadence tick that finds another core's running skips, and each core's cadence starts `core_id / cores` of a period late so the ticks do not meet every period. A skip costs that core's contribution to the fold a later redo start, never an answer. Every core publishes into the one `SuperBlockCheckpointAnchor` **itself** since AT-S8 — `RemoteCheckpointAnchor` and ring kind 16 retired — under the superblock latch `rules.md` §3 declares. Under one stream the two checkpoint records name their logging core in the record envelope's `flags` byte, since the log now carries every core's, and step 3 publishes into the fold of §3 rather than into a slot of this core's own:
...
9. **Topology:** a peer's writes are recovered by core 0's single pass and a peer runs none — plus the shared-stream cells: concurrent appenders each landing at the LSN they were handed, no LSN twice, the durable watermark never retreating, and a peer's `strict` commit made durable by core 0's writer with the peer's own sync count at zero.
