...
- **No prepare is ever written since AT-S6; the mount still resolves one a pre-AT volume carries.** 2PC is retired (`cross-owner-txn.md`), and it had stopped writing before the stage ran: SA-T0 made a participant that wrote nothing skip the record, and AT-S5 left no participant that could write. AT-S18 deleted the emitter, the runtime prepared state and §11-3's prepare floor. What stays is the mount scan's arm for a volume written before then: the coordinator's decision is a record of the same log as the participant's `TXN_PREPARE`, so a commit makes the participant a winner and **absence of a decision is abort** - sound because the engine that wrote the prepare floored every checkpoint's redo start at its oldest live one. The pass resolves every prepare before its completion checkpoint, so no checkpoint of this engine sees one live, which is why the floor could go. `SHOW META` prints `recovery_prepared*` only when a mount resolved one. Cells: `mount_recovery_test.cpp`'s two pre-AT cells, `wal_recovery_test.cpp`'s resolution cells, `prepared_recovery_test.cpp`'s analysis cells.
...
- **The core count is not pinned since AT-S9.** The superblock records `core_count`, and a mount at a different `cores` records the new count and logs the change rather than refusing (`src/bootstrap/bootstrap.cpp`, `SuperBlock::SetCoreCount`). Every reason it was pinned is gone: per-core streams ended at AM-S4(d), and `owner_core`, the last thing on the volume that named a core, was dropped at AT-S9 (D17). What is sized by the count - the anchor fold's warm-up over every core's published checkpoint, the lock table's partitions, the page latch's owner bits - is sized from the running one, and the fold's slot 0 holds the lowest redo start whatever count wrote it, so recovery replays from it correctly across the change. Online core-count change - adding a core to a running instance - is not supported.
