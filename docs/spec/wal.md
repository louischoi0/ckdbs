...
- **The core count is pinned.** The superblock records `core_count` at bootstrap and a mount whose running `cores` disagrees is refused, naming both numbers (`superblock.hpp`). Under per-core streams the reason was that a mount at M would leave |N − M| streams with nothing to replay them. Under one stream it is different and no weaker: the anchor's warm-up is defined over "every core has published", so a changed count would either park the fold at the mount anchor forever or advance it past a core that no longer runs. Online core-count change is not supported.
