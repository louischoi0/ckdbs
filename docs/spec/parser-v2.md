...
**`[AMENDED 2026-08-31 — J2's one code became two]`** Everywhere this document writes `Unsupported`, read "refused, understood, with an exact position" — and read the *code* off the operator rule that split it (`include/kds/base/status.hpp`, `docs/spec/protocol.md` §11): `Unsupported` is what this engine's architecture cannot admit and no later release lifts, `NotImplemented` is what the design admits and nobody built. The position requirement, the never-a-generic-path requirement and every form's membership in J2 are untouched; only which of the two codes each form carries is new. **Most of J2 is `NotImplemented`** — table-position nesting (derived tables and CTEs: the "execution model, not an unimplemented feature" argument expired when `exec::InnerBuild` gave the engine a statement-scoped row set probed by a non-pk key, `join-inner-build.md`), outer joins, `ORDER BY` by ordinal or expression, the aggregated-output tail, subqueries containing aggregates, and `ORDER BY`/`LIMIT` inside a subquery. **What stays `Unsupported`** is the shorter list: the depth cap (a recursion needs *a* bound structurally, and the parser, the compiler and the step VM agree on it), one relation named twice in a FROM list without distinct aliases (distinct aliases are the answer on every later release too), and a `$name` parameter on the newline protocol, whose wire has no bind step. The corpus (`tests/testdata/parser_corpus.txt`) is the enumeration, verdict by verdict.
...
- **Derived conjuncts are marked** (`StepPredicate::derived`): `ANALYZE` prints `derived` on their filter lines — a diagnostic must name a predicate the client can find in their text, and the derived occurrence's verdict is never different, since the descriptor guard makes both columns the same type. `CREATE PATTERN`'s parameter checks were the second consumer of the mark, for the same reason, until that statement was withdrawn on 2026-08-31.
