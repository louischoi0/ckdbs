...
- **V11** — `CREATE TABLE ... WITH (PHYSICAL_OPTIMIZER = ON|OFF)`. No option table exists in the parser, which is why `docs/spec/feat-physical-optimizer.md` R12 says "once both exist".
...
- **Still `f(shape, catalog)`.** First-seen order everywhere, no data consulted; `docs/spec/feat-index.md` §9's purity claim is unchanged.
...
- **Read scheduling is unchanged.** The inner relation is first read exactly when written order reads it: the build is that walk's side effect, the Recording pattern `docs/spec/feat-cabin.md` §4 ratified.
