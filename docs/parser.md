...
The parser runs at KWP `C_PARSE` only — never per execution. It is therefore off the per-execution hot path, but it is the **front door of every engine differentiator**: fingerprints for Waystone, statement classes for plan-free dispatch, and the DDL surface for engine features all originate here.

---

## 1. Architecture Instructions

**I1 — Parameterize literals at parse time (pattern_id for free).**
...
**I2 — Tag statement classes (switch dispatch, no plan search).**
...
**I3 — Per-session arena + index-based flat AST (zero alloc).**
...
**I4 — Zero-copy tokenizing via string_view over KWP frames.**
...
**I5 — Catalog binding at parse + DDL version-stamp invalidation.**
...
**I6 — Enforce the 40-bit pk literal range (invariant 6 at the front door).**
...
**I7 — KDS DDL surface: CREATE TABLE WITH options.**
`CREATE TABLE ... WITH (key = value, ...)` with an extensible option table. Unknown options are a parse error naming the option. Future engine features add options — never new syntax.

The v1 option is `PHYSICAL_OPTIMIZER = ON|OFF`, a per-relation gate. **Waystone is deliberately not an option here**: it is keyed on `(pattern_id, arg_hash)`, not on a relation, so there is no per-relation flag for the grammar to set. Also note the storage form (`HEAP`/`BTREE`) is a trailing clause in today's grammar rather than a `WITH` option; folding it in is a grammar decision this item does not take.
*Done when:* the option round-trips into a catalog flag; adding an option requires no grammar change.

**I8 — Session/admin statements.**
...
**I9 — Join scope: inner equi-join chains; reserve outer syntax only.**
...
**I10 — Filter scope: AND-combined comparisons + IN, BETWEEN.**
...
**I11 — ORDER BY pk + LIMIT (pagination is mandatory in v1).**
...
**I13 — Dialect compatibility is a non-goal.**
...
**I12 — Join order = written order ("the query is the plan").**
...
3. `docs/waystone-concpets.md`: `kJoinSelect` is the statement class a cross-relation waystone trail is recorded for.
