...
**Known gap — dividing a full *internal* node is not implemented.** When a division promotes a separator into a parent that is already full, the parent right-splits with no movement: a new node whose only child is the new subtree, with the separator promoted another level. **That is sound only when the separator sorts above every separator the node already holds** — an argument that used to be free, because a monotonic sequence always split at the rightmost edge, and that a caller-supplied id removes. It is now checked rather than assumed, because promoting an interior separator that way would strand every subtree sorting above it: silent data loss, not a wrong answer anyone would notice. A separator that sorts below the node's highest is refused with `OutOfSpace` naming the gap (`PromoteSeparator`). Reaching it needs roughly 678 leaf divisions under one parent — `kInternalMaxEntries` is 678 — so it is remote, not impossible. Recorded in `docs/known-gaps.md` and left as remaining work in `docs/workplan-key-mode.md`.
...
- **Dividing a full btree internal node** is `[OPEN]` and unbuilt (§4.1). A separator promoted into a full internal node right-splits with no movement, which is sound only when it sorts above every separator that node holds; below it, the insert is refused with `OutOfSpace` naming the gap rather than stranding the subtrees above it. Needs ~678 leaf divisions under one parent to reach.
