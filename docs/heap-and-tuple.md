...
**What silence means is configurable** (`default_key_mode`, shipped `assigned`). A statement that names no key-mode word takes the instance's setting; a statement that names one always wins over it, or the statement would not mean what it says. The setting exists so a database whose keys come from outside — a migration, an upstream system, natural keys — says so once rather than on every `CREATE TABLE`.

Setting it to `explicit` also moves the **storage** default for a statement that named neither word, heap → btree, since an explicit relation must be btree-clustered. That is the one place a key-mode setting reaches past key mode, and it is there so the default is usable: without it every unqualified `CREATE TABLE` on such an instance would be refused, which is not a default. A written storage word still wins, including one that contradicts the mode — `... HEAP` under an explicit default is still the `Unsupported` above, because that pairing came from the writer and not from resolution.

Worth stating for anyone considering it instance-wide: an all-explicit database gives up the semi-sorted heap, and with it **every relayout candidate** — `stats/relayout_planner.cpp` builds plans only for heap-clustered non-system relations, so the physical optimizer has nothing to act on. Waystone is unaffected, being storage-agnostic. The catalog's own relations stay `ASSIGNED` regardless: six sites issue ids for engine-internal objects, and there is no caller to supply those.
...
**`ORDER BY <pk>` emits each page in key order on an `EXPLICIT` relation.** `exec::CompileStepChain` accepts the driving relation's pk as an `ORDER BY` target, and for a monotonic sequence it can discard the clause outright: a walk emits a page's slots consecutively in slot order (`RunWalkStep`, `src/exec/step_vm.cpp`), and an engine-issued id is appended above every id already on the page, so slot order *is* key order. A supplied id can be appended below them, so on an `EXPLICIT` relation the two diverge — **within one page only**, since page-wise `min_key` ordering is preserved by a division. So the clause sets `Step::emit_in_key_order` rather than being discarded, and the walk emits that page's live slots sorted by Keystone id. It is asked for rather than always on: the sort reads every live slot's key up front, where the natural walk reads one per emitted row, and a statement that did not ask for an order should not pay for one. `LIMIT`/`OFFSET` is a prefix of the emitted order and needs nothing of its own.
