...
public:
    ...
    // This transport has no wake half at all, and since AU-S1b there is no
    // no-op to write for one: a simulated reactor is pumped by the
    // simulation loop and never blocks in a real backend, so it has no sleep
    // to interrupt, and the kick is `RealRingTransport`'s own call into the
    // instance's `WakerTable` rather than anything this interface carries.
