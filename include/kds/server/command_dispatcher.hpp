...
private:
    ...
    // The tail every converted DDL handler runs: uninstalls the undo
    // hook, resolves the write scope (commit/abort for an owned one), and
    // for an owned scope runs the DDL-resolution seam - cache
    // invalidation and the §5d purge - that explicit COMMIT/ROLLBACK
    // reaches through EndDdlScope.
