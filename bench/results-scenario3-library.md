...
| **What engine this is** | `dd32cb8`'s engine source is **byte-identical to `ed03b44`**, which is the commit now tagged **`v1.0.0`** — `git diff ed03b44 dd32cb8 -- src/ include/ tests/` is empty. These numbers therefore describe the tagged release, not a tree adjacent to it. Stated against `dd32cb8` rather than `HEAD` on purpose: `main` has since taken the recovery series (RC07–RC11), so the engine at `HEAD` is no longer the engine these numbers were measured on. |
