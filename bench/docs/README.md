...
| `cabin_scope_ab_benchmark.py` | SB-M1 (`instructions/v2.7.1/workorder-sb.md`): whether a **one-range relation**'s Cabin serve path is cycle-neutral after SB1-SB4 scoped Cabin authority to (observed value × owned ranges), H-SB1's claim that `CabinScopeCovers`'s `access.ranges.empty()` fast path costs nothing (`src/exec/step_vm.cpp`). An interleaved A/B across **two binaries**, never one server with a switch — the scoping is compiled in — `--port`/`--server-pid` the side under test, `--ab-port`/`--ab-server-pid` the pre-SB baseline. Four relations per size per side: `{ch}`/`{cb}` (heap/btree, `CREATE CABIN`-declared on `val`, `kDeclaredRecordThreshold = 1` — two untimed warmup probes per hot value, the first records and the second is the first hit, and one `ANALYZE` per cabined relation must show `cabin_hits=1` before anything is counted) and `{uh}`/`{ub}` (heap/btree, no Cabin at all — **the control**, since `CabinScopeCovers` is reachable only from `RunCabinStep`, which a column with no declared Cabin never compiles into; a delta here is the host, not SB). Arms: `ping` (`SHOW META`, round-trip floor), `heap-cabin` / `heap-cabin-again` (the second is the in-run noise floor), `heap-ctl`, `btree-cabin`, `btree-ctl`. `--rows` is a **comma-separated list** swept inside one invocation (default `200,1000,10000`, rule 9's floor) — one server pair, one relation quartet per size, so a fixed per-branch cost reads flat across three cardinalities 50x apart and a per-row one would not. Server CPU from a second pass of contiguous `/proc/<pid>/stat` windows (`--cpu-ops` × `--cpu-rounds`), never summed over the latency pass. `--verify` (default on) runs eleven queries per size — a cabin hit, a miss, an equality with an extra conjunct, a pk lookup, `COUNT(*)`, a correlated self-join through the Cabin, and the uncabined controls' twins, shapes borrowed from `tests/cabin_contract_test.cpp`'s `Queries()` — against both sides and diffs every reply field for field. Refuses a box whose load average is high or that is mid-compile, `--force` overrides. No PostgreSQL twin — an observed-value entry set has no PostgreSQL counterpart. `--port 15922 --ab-port 15921 --server-pid P --ab-server-pid Q --rows 200,1000,10000 --ops 3000 --cpu-ops 1500 --cpu-rounds 4 --suffix s1 --json out.json` |
