...
- **The pk is system-generated, always.** Issued by `Catalog::AllocateRowId()` from a *persistent* `sys.tables.next_id`, never derived as `max(id) + 1` — deriving it would reissue the identity of a deleted row. A caller-supplied pk on insert is a defect, not a feature.
...
- **The pk is stored once.** Carried only by the Keystone word, never also as a body column — two copies are how two copies come to disagree.
- **Ids are unique and monotonic, not gapless.** A failed insert burns one. Nothing depends on gaplessness.
...
A relation is stored either as this heap chain or as a **clustered B+ tree** on the pk, chosen at `CREATE TABLE`. A btree leaf *is* a heap page — same slots, same tuple format, same MVCC header — so the tree is a directory over the heap, not a second storage engine.
...
| 11 | Every pk is system-generated autoincrement, carried only by the Keystone word, never updatable | Issue-once identity |
...
**Read [`docs/known-gaps.md`](docs/known-gaps.md) before relying on anything.** It is the engine-wide list of what is missing and what a restart loses, kept deliberately blunt. The headline entries: **WAL recovery is not implemented** (the log is written and never read back); nothing purges anywhere, because readers are deliberately unregistered; `cores > 1` buys parallel WAL streams only, since core 0 still serves every statement until the step pipeline lands; Cabin entry sets, the assertion registry, and the Cabin controller's managed state are memory-resident and rebuilt by traffic after a restart; and there is no auth, no TLS, loopback only.
