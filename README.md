...
- **The pk is unique for the life of the relation, and where it comes from is the relation's choice** — its **key mode**, fixed at `CREATE TABLE` (`docs/heap-and-tuple.md` §4.1, built 2026-08-11). `ASSIGNED`, the default, draws it from a *persistent* per-relation cursor, `sys.tables.next_id`, never derived as `max(id) + 1` — deriving it would reissue the identity of a deleted row. `EXPLICIT` takes it from the caller.
- **Under `ASSIGNED` the pk ascends; under `EXPLICIT` it need not.** The two prove uniqueness differently, and that is the whole difference: the cursor proves it without reading a page, the clustered btree's descent proves it by landing on the one leaf that may hold the key. Which is why an `EXPLICIT` relation must be `BTREE`-clustered, and why the semi-sorted heap chain — which has no descent — is `ASSIGNED`-only and keeps ascending ids unconditionally.
- **The pk cannot be updated, in either mode.** It is the tuple's identity, not a field of it; an `UPDATE` naming it is refused at compile time as `Unsupported`, with the byte position of the column. Naming a key at insert and changing one afterwards are unrelated permissions; only the first was granted.
...
A relation is stored either as this heap chain or as a **clustered B+ tree** on the pk, chosen at `CREATE TABLE` — and forced to the tree when the key mode is `EXPLICIT`, because only a descent can place and prove a key the caller named. A btree leaf *is* a heap page — same slots, same tuple format, same MVCC header — so the tree is a directory over the heap, not a second storage engine. The tree is the one place a page's tuples divide: an id sorting inside a full leaf moves the upper half out to a new leaf, the old leaf keeping its `min_key` and the new one taking the split key, so both invariants above survive the move.
...
| **B+ tree** | The authoritative pk → location index. One tree core, thin facades; append-optimized for monotonic engine-issued ids (rightmost fast path, asymmetric splits), and dividing a full leaf at its median key when an `EXPLICIT` relation's caller names one out of order. Core-local — no latching protocols at all |
...
| 11 | Every pk is a unique 40-bit id, carried only by the Keystone word, **never updatable**; the engine issues it (`ASSIGNED`) or the caller names it (`EXPLICIT`, btree-only, order-free) | Issue-once identity |
